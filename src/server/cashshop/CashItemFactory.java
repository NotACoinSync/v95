...
public class CashItemFactory {
            ...
            if (ServerConstants.WZ_LOADING) {
                ...
                for (MapleData item : etc.getData("Commodity.img").getChildren()) {
                    ...
                    if (ServerConstants.BIN_DUMPING) {
                        data.saveToBin(oPacket);
                    }
                }
                if (ServerConstants.BIN_DUMPING) {
                    commodity.createNewFile();
                    oPacket.saveToFile(commodity);
                }
                for (MapleData cashPackage : etc.getData("CashPackage.img").getChildren()) {
                    ...
                    itemPackages.put(Integer.parseInt(cashPackage.getName()), cPackage);
                }
                if (ServerConstants.BIN_DUMPING) {
                    File binFile = new File(binBase, "/CashPackage.bin");
                    if (!binFile.exists()) {
                        oPacket = new MaplePacketLittleEndianWriter();
                        oPacket.writeInt(itemPackages.size());
                        for (int id : itemPackages.keySet()) {
                            oPacket.writeInt(id);
                            List<Integer> ids = itemPackages.get(id);
                            oPacket.writeInt(ids.size());
                            for (int i : ids) {
                                oPacket.writeInt(i);
                            }
                        }
    ...
    public static List<Item> getPackage(int itemId) {
        ...
        for (int sn : itemPackages.get(itemId)) {
    ...
    public static boolean isPackage(int itemId) {
        return itemPackages.containsKey(itemId);
    }
