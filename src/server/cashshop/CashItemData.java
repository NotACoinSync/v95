...
public class CashItemData {
        ...
        if (packageData != null && packageData.length() > 0) {
            for (String s : packageData.split(",")) {
                if (s != null && s.length() > 0) {
                    packageSN.add(ObjectParser.isInt(s));
                }
            }
        }
        CashItemData ci = CashItemFactory.getItem(sn);
        if (ci != null) {
            if (ci.nPriority != nPriority) {
                modifier |= CashModification.PRIORITY;
            }
            if (ci.nCommodityGender != nCommodityGender) {
                modifier |= CashModification.COMMODITYGENDER;
            }
            if (ci.nClass != nClass) {
                modifier |= CashModification.CLASS;
            }
            if (ci.nLimit != nLimit) {
                modifier |= CashModification.LIMIT;
            }
            if (ci.bBonus != bBonus) {
                modifier |= CashModification.BONUS;
            }
            if (ci.bForPremiumUser != bForPremiumUser) {
                modifier |= CashModification.FORPREMIUMUSER;
            }
            if (ci.bOnSale != bOnSale) {
                modifier |= CashModification.ONSALE;
            }
            if (ci.nCount != nCount) {
                modifier |= CashModification.COUNT;
            }
            if (ci.nPeriod != nPeriod) {
                modifier |= CashModification.PERIOD;
            }
            if (ci.nReqPOP != nReqPOP) {
                modifier |= CashModification.REQPOP;
            }
            if (ci.nReqLEV != nReqLEV) {
                modifier |= CashModification.REQLEV;
            }
            if (ci.nPbCash != nPbCash) {
                modifier |= CashModification.PBCASH;
            }
            if (ci.nPbPoint != nPbPoint) {
                modifier |= CashModification.PBPOINT;
            }
            if (ci.nPbGift != nPbGift) {
                modifier |= CashModification.PBGIFT;
            }
            if (ci.nItemid != nItemid) {
                modifier |= CashModification.ITEMID;
            }
            if (ci.nPrice != nPrice) {
                modifier |= CashModification.PRICE;
            }
            if (ci.nMaplePoint != nMaplePoint) {
                modifier |= CashModification.MAPLEPOINT;
            }
            if (ci.nMeso != nMeso) {
                modifier |= CashModification.MESO;
            }
            if (ci.packageSN.size() != packageSN.size()) {
    ...
    public void encode(LittleEndianWriter lew) {
        ...
        if ((modifier & CashModification.ITEMID) > 0) {
            lew.writeInt(nItemid);
        }
        if ((modifier & CashModification.COUNT) > 0) {
            lew.writeShort(nCount);
        }
        if ((modifier & CashModification.PRIORITY) > 0) {
            lew.write(nPriority);
        }
        if ((modifier & CashModification.PRICE) > 0) {
            lew.writeInt(nPrice);
        }
        if ((modifier & CashModification.BONUS) > 0) {
            lew.writeBoolean(bBonus);
        }
        if ((modifier & CashModification.PERIOD) > 0) {
            lew.writeShort(nPeriod);
        }
        if ((modifier & CashModification.REQPOP) > 0) {
            lew.writeShort(nReqPOP);
        }
        if ((modifier & CashModification.REQLEV) > 0) {
            lew.writeShort(nReqLEV);
        }
        if ((modifier & CashModification.MAPLEPOINT) > 0) {
            lew.writeInt(nMaplePoint);
        }
        if ((modifier & CashModification.MESO) > 0) {
            lew.writeInt(nMeso);
        }
        if ((modifier & CashModification.FORPREMIUMUSER) > 0) {
            lew.writeBoolean(bForPremiumUser);
        }
        if ((modifier & CashModification.COMMODITYGENDER) > 0) {
            lew.write(nCommodityGender);
        }
        if ((modifier & CashModification.ONSALE) > 0) {
            lew.writeBoolean(bOnSale);
        }
        if ((modifier & CashModification.CLASS) > 0) {
            lew.write(nClass);
        }
        if ((modifier & CashModification.LIMIT) > 0) {
            lew.write(nLimit);
        }
        if ((modifier & CashModification.PBCASH) > 0) {
            lew.writeShort(nPbCash);
        }
        if ((modifier & CashModification.PBPOINT) > 0) {
            lew.writeShort(nPbPoint);
        }
        if ((modifier & CashModification.PBGIFT) > 0) {
            lew.writeShort(nPbGift);
        }
