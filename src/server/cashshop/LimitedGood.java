...
public class LimitedGood {
    ...
    public int getState() {
        ...
        if (nRemainCount <= 0) {
            return LimitGoodsState.CountLimited;
        }
        if (calendar.get(Calendar.HOUR_OF_DAY) < nHourStart || calendar.get(Calendar.HOUR_OF_DAY) > nHourEnd) {
            return LimitGoodsState.HourLimited;
        }
        if (calendar.get(Calendar.DAY_OF_MONTH) < nDateStart || calendar.get(Calendar.DAY_OF_MONTH) > nDateEnd) {
            return LimitGoodsState.DateLimited;
        }
        if (!abWeek[calendar.get(Calendar.DAY_OF_WEEK) - 1]) {
            return LimitGoodsState.WeekLimited;
        }
        return 0;
    }

    public int getStockState(CashItemData data) {
        if (nRemainCount >= Integer.MAX_VALUE) {
            return StockState.Infinite;
        }
        if (nRemainCount < data.nCount) {
            return StockState.NoStock;
        }
        Calendar calendar = Calendar.getInstance();
        if (calendar.get(Calendar.HOUR_OF_DAY) < nHourStart || calendar.get(Calendar.HOUR_OF_DAY) > nHourEnd) {
            return StockState.NotAvailableTime;
        }
        if (calendar.get(Calendar.DAY_OF_MONTH) < nDateStart || calendar.get(Calendar.DAY_OF_MONTH) > nDateEnd) {
            return StockState.NotAvailableTime;
        }
