...
public class MapleQuest {
        ...
        if (binFile.exists() && !ServerConstants.WZ_LOADING) {
            ...
            try {
                if (ServerConstants.BIN_DUMPING) {
                    ...
                    oPacket.writeInt(timeLimit2);
                    oPacket.writeBoolean(autoStart);
                    oPacket.writeBoolean(autoPreComplete);
                    oPacket.writeBoolean(autoComplete);
                    // this is after startReq/ completeReq
                    oPacket.writeBoolean(repeatable);
                    //
                    startQuestData.encode(oPacket);
                    completeQuestData.encode(oPacket);
                    oPacket.saveToFile(binFile);
                }
            } catch (IOException e) {
                Logger.log(LogType.ERROR, LogFile.EXCEPTION, e);
            }
    ...
    public boolean canStart(MapleCharacter c, int npcid) {
        ...
        for (MapleQuestRequirement r : startQuestData.startReqs.values()) {
            if (!r.check(c, npcid)) {
                if (c.getScriptDebug()) {
                    c.dropMessage(MessageType.MAPLETIP, "Failed requirement: " + r.toString());
                }
                return false;
            }
        }
    ...
    public void start(MapleCharacter c, int npc) {
        if (autoStart || canStart(c, npc)) {
            ...
            if (c.getScriptDebug()) {
                c.dropMessage(MessageType.MAPLETIP, "Quest start(failed) ID: " + this.id + ", NPC: " + npc);
            }
        }
    }
    ...
    public void forfeit(MapleCharacter c) {
        ...
        if (timeLimit > 0) {
            c.announce(UserLocal.QuestResult.removeQuestTimer(id));
        }
    ...
    public boolean forceStart(MapleCharacter c, int npc) {
        if (c.getScriptDebug()) {
            c.dropMessage(MessageType.MAPLETIP, "Quest Force Start ID: " + this.id + ", NPC: " + npc);
        }
    ...
    public boolean forceComplete(MapleCharacter c, int npc, boolean sendCompletePacket) {
        ...
        c.updateQuest(newStatus);
        if (sendCompletePacket) {
            c.announce(WvsContext.QuestClear(getId()));
        }
    ...
    public static int getQuests() {
        return quests.keySet().size();
    }

    /**
     * Gets the {@link MapleQuestRequirement} based on the
     * {@link MapleQuestRequirementType} provided. Also initializes the action
     ...
     * @return {@link MapleQuestRequirement} The initialized class for the
     * requirement.
     */
