...
public class MapleInventoryManipulator {
    ...
    /**
     * Use {@link MapleCharacter.canHoldItem(item)} To check if the item can be
     * held before using this method.
     *
     * @return False if 1 or mores items failed to add, or no room was
     * available.
     */
    ...
    public static boolean addFromDrop(MapleClient c, Item item, boolean show, boolean log) {
        ...
        if (type.equals(MapleInventoryType.EQUIP)) {
            ...
            c.announce(WvsContext.InventoryOperation(true, Collections.singletonList(new ModifyInventory(0, item))));
            if (show) {
                c.announce(EffectPacket.Local.Quest(item.getItemId(), item.getQuantity()));
            }
        } else {
            if (quantity > 0) {// update other items
                    ...
                    for (Item exist : c.getPlayer().getInventory(type).listById(item.getItemId())) {
                        if (exist.getQuantity() < max) {
                            ...
                            if (qLeft > 0) {// can add items
                                ...
                                c.announce(WvsContext.InventoryOperation(true, Collections.singletonList(new ModifyInventory(1, exist))));
                                if (show) {
                                    c.announce(EffectPacket.Local.Quest(item.getItemId(), add));
                                }
                            }
                        }
                    }
                ...
                c.announce(WvsContext.InventoryOperation(true, Collections.singletonList(new ModifyInventory(0, nItem))));
                if (show) {
                    c.announce(EffectPacket.Local.Quest(item.getItemId(), nItem.getQuantity()));
                }
            }
            if (quantity == 0) {
            ...
            while (quantity > 0) {
                ...
                if (ItemConstants.isRechargable(nItem.getItemId())) {
		...
		if(!type.equals(MapleInventoryType.EQUIP)){
			...
			if(!ItemConstants.isRechargable(item.getItemId())){
					...
					while(quantity > 0){
						if(i.hasNext()){
							...
							if(oldQ < slotMax && item.getOwner().equals(eItem.getOwner())){
								...
								c.announce(MaplePacketCreator.InventoryOperation(true, Collections.singletonList(new ModifyInventory(1, eItem))));
							}
						}else{
							break;
						}
				...
				while(quantity > 0){
					...
					c.announce(MaplePacketCreator.InventoryOperation(true, Collections.singletonList(new ModifyInventory(0, nItem))));
				}
			}else{
			...
			c.announce(MaplePacketCreator.InventoryOperation(true, Collections.singletonList(new ModifyInventory(0, item))));
		}else{
			return false;
		}
		if(show){
    ...
    /**
     * Removes the specified amount of quantity of an item. Will check the slot
     * defined first, then if the required amount of quantity to remove is still
     * greater than zero, move onto checking the entire inventory.
     *
    ...
    public static void removeItem(MapleClient c, MapleInventoryType type, int slot, int quantity, boolean deleteIfZero, boolean fromDrop, boolean consumeIfZero) {
        ...
        if (item != null) {
            ...
            if (item.getQuantity() == 0 && consumeIfZero) {
                ...
                c.announce(WvsContext.InventoryOperation(fromDrop, Collections.singletonList(new ModifyInventory(3, item))));
            } else {
                c.announce(WvsContext.InventoryOperation(fromDrop, Collections.singletonList(new ModifyInventory(1, item))));
            }
            if (quantityLeft > 0) {
    ...
    public static void removeStarById(MapleClient c, MapleInventoryType type, int itemId, int quantity, boolean fromDrop, boolean consumeIfZero) {
        ...
        for (short i = 0; i <= slotLimit; i++) {
            ...
            if (item != null) {
                if (item.getItemId() == itemId || item.getCashId() == itemId) {
                    if (item.getQuantity() == 0 || removeQuantity <= 0) {
                        continue;
                    }
    ...
    public static void equip(MapleClient c, short src, short dst) {
        ...
        if (c.getPlayer().getBuffedValue(SecondaryStat.Booster) != null && isWeapon(source.getItemId())) {
            c.getPlayer().cancelBuffStats(SecondaryStat.Booster);
        }
        mods.add(new ModifyInventory(2, source, src));
        c.announce(WvsContext.InventoryOperation(true, mods));
    ...
    public static void unequip(MapleClient c, short src, short dst) {
        ...
        c.announce(WvsContext.InventoryOperation(true, Collections.singletonList(new ModifyInventory(2, source, src))));
        c.getPlayer().equipChanged();
    }
    ...
    public static void drop(MapleClient c, MapleInventoryType type, short src, short quantity) {
        ...
        if (c.getPlayer().getItemEffect() == itemId && source.getQuantity() == 1) {
            ...
            c.getPlayer().getMap().announce(UserRemote.setActiveEffectItem(c.getPlayer().getId(), 0));
        ...
        if (quantity < source.getQuantity() && !ItemConstants.isRechargable(itemId)) {
            ...
            c.announce(WvsContext.InventoryOperation(true, Collections.singletonList(new ModifyInventory(1, source))));
            ...
            c.announce(WvsContext.InventoryOperation(true, Collections.singletonList(new ModifyInventory(3, source))));
            if (src < 0) {
                c.getPlayer().equipChanged();
            }
