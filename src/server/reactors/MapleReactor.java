...
public class MapleReactor extends AbstractMapleMapObject {
    ...
    public void save(MaplePacketLittleEndianWriter oPacket) {
        oPacket.writeInt(rid);
        oPacket.writeInt(link);
        oPacket.writeInt(delay);
        oPacket.writeInt(quest);
        oPacket.writeInt(runeid);
        oPacket.writeBoolean(action != null);
        if (action != null) {
            oPacket.writeMapleAsciiString(action);
        }
        oPacket.writeBoolean(alive);
        oPacket.writeBoolean(canSetAlive);
        oPacket.writeInt(states.size());
    ...
    public void load(LittleEndianAccessor slea) {
        ...
        runeid = slea.readInt();
        if (slea.readBoolean()) {
            action = slea.readMapleAsciiString();
        }
    ...
    public void setState(Integer state) {
        ...
        checkForTimeout();
        if (getMap() != null) {
            getMap().announce(ReactorPool.ReactorChangeState(this, 2));
        }
    }
    ...
    public final byte[] makeDestroyData() {
        return ReactorPool.ReactorLeaveField(this);
    }
    ...
    public final byte[] makeSpawnData() {
        return ReactorPool.ReactorEnterField(this);
    }
    ...
    public void hitReactor(MapleClient c) {// legacy shit
        try {
            if (checkQuest(c)) {
                states.get(getCurrState()).runEvents(c, this, ReactorHitType.HIT, new ReactorHitInfo());
            }
        } catch (Exception ex) {
    ...
    public void hitReactor(ReactorHitType type, ReactorHitInfo info, MapleClient c) {
        try {
            if (checkQuest(c)) {
                if (c != null && c.getPlayer() != null && c.getPlayer().getScriptDebug()) {
                    c.getPlayer().dropMessage(MessageType.MAPLETIP, "Reactor: " + getId() + " action: " + getAction());
                }
    ...
    private boolean checkQuest(MapleClient c) {
        ...
        if (quest > 0) {
            ...
            if (status.getStatus().equals(MapleQuestStatus.Status.STARTED)) {
                return true;
            } else {
                return false;
            }
