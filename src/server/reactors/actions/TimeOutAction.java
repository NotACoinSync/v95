...
public class TimeOutAction extends MapleReactorEvent {
    ...
    public void run(MapleClient c, MapleReactor reactor, ReactorHitInfo info) {
        if (reactor.getCurrState() != stateTo) {
            if (lastRun == -1 || (System.currentTimeMillis() - lastRun >= timeout)) {
                ...
                if (reactor.canSetAlive() && !reactor.isAlive()) {
                    reactor.setAlive(true);
                }
                if (reactor != null && reactor.getMap() != null && reactor.getState() != null) {
                    reactor.getMap().activateItemReactors(c);
                }
            } else {
    ...
    public void save(MaplePacketLittleEndianWriter oPacket) {
        oPacket.writeInt(timeout);
        oPacket.writeInt(stateTo);
    }
