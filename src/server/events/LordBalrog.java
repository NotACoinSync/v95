...
/**
 * @author Julien bottom reactor : 1058004, left : 1058001, right : 1058003
 */
public class LordBalrog {
    ...
    /*private void startQuest(){
		getLobbyInstance().announce(MaplePacketCreator.serverNotice(5, "The portal to Lord Balrog's tomb is now open. Any member of '" + group.getName() + "' may enter."));
	}*/
 ...
 /*private class SleepSchedule implements Runnable{
		...
		@Override
		public void run(){
			getMapInstance().announce(MaplePacketCreator.makeMonsterInvisible(getBalrog()));
			...
			getMapInstance().announce(MaplePacketCreator.triggerReactor(target, 0)); // assuming stance = 0;..
	...
	private class warpOut implements Runnable{
		...
		public void run(){
			...
			net.server.Server.getInstance().announce(MaplePacketCreator.serverNotice(0, "Despite their best efforts, " + group.getName() + " was unable to prevent Lord Balrog from reviving."));
			dispose();
		}
	}*/
    public void dispose() {
