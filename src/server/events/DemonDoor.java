    ...
    public void warpParty() {
        for (MaplePartyCharacter mpc : group.getMembers()) {
            ...
            if (chr != null) {
                ...
                chr.announce(WvsContext.ScriptProgressMessage("Brace yourselves! It is coming..."));
                chr.announce(Clock.Created(10, 2));
            }
    ...
    public class start implements Runnable {
        ...
        public void run() {
            for (MaplePartyCharacter mpc : group.getMembers()) {
                MapleCharacter chr = mpc.getPlayerInChannel();
                if (chr != null) {
                    chr.announce(Clock.Created((1000 * 60 * 20) - 1000 * 10, 2));
                }
            }
            Point spawnPoint;
