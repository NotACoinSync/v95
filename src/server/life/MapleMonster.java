...
public class MapleMonster extends AbstractLoadedMapleLife {
                    ...
                    if (updateTime - lastPoisonDamage >= 1000 && poisonType != -1) {
                        ...
                        if (damage >= hp) {
                            ...
                            if (poisonType == 1 || poisonType == 2) {
                                map.announce(MobPool.damageMonster(getObjectId(), damage), getPosition());
                            }
                        }
                        if (hp > 1 && damage > 0) {
    ...
    public void damage(MapleCharacter from, int damage, boolean updateAttackTime) {
        ...
        if (hasBossHPBar()) {
            from.getMap().announce(makeBossHPBarPacket(), getPosition());
        } else if (!isBoss()) {
                ...
                for (AttackingMapleCharacter cattacker : mattacker.getAttackers()) {
                    if (cattacker.getAttacker().getMap() == from.getMap()) {
                        if (cattacker.getLastAttackTime() >= okTime) {
                            cattacker.getAttacker().getClient().announce(MobPool.HPIndicator(getObjectId(), remhppercentage));
                        }
                    }
                }
            }
    ...
    public void heal(int hp, int mp) {
        ...
        setHp(hp2Heal);
        setMp(mp2Heal);
        getMap().announce(MobPool.healMonster(getObjectId(), hp));
    }
    ...
    public MapleCharacter killBy(MapleCharacter killer) {
        ...
        if (toSpawn != null) {
            ...
            if (toSpawn.contains(9300216) && reviveMap.getId() > 925000000 && reviveMap.getId() < 926000000) {
                reviveMap.announce(Field.FieldEffect.playSound("Dojang/clear"));
                reviveMap.announce(Field.FieldEffect.showEffect("dojang/end/clear"));
            }
            Pair<Integer, String> timeMob = reviveMap.getMapData().getTimeMob();
            if (timeMob != null) {
                if (toSpawn.contains(timeMob.getLeft())) {
                    reviveMap.announce(WvsContext.BroadcastMsg.encode(6, timeMob.getRight()));
                }
                if (timeMob.getLeft() == 9300338 && (reviveMap.getId() >= 922240100 && reviveMap.getId() <= 922240119)) {
                    if (!reviveMap.containsNPC(9001108)) {
                        ...
                        reviveMap.announce(NpcPool.NpcEnterField(npc));
                    } else {
    ...
    public void switchController(MapleCharacter newController, boolean immediateAggro) {
        ...
        if (controllers != null && controllers.getId() == newController.getId()) {
            return;
        }
        if (controllers != null) {
    ...
    public byte[] makeBossHPBarPacket() {
        return Field.FieldEffect.showBossHP(getId(), getHp(), getMaxHp(), getTagColor(), getTagBgColor());
    }
    ...
    public ElementalEffectiveness getEffectiveness(Element e) {
        if (mobStats.size() > 0 && mobStats.containsKey(MobStat.Doom)) {
    ...
    public boolean applyStatus(MapleCharacter from, Map<MobStat, MobStatData> stats, Skill skill, boolean poison, boolean venom) {
        ...
        if (this.stats.isBoss()) {
            if (!(stats.containsKey(MobStat.Speed) && stats.containsKey(MobStat.Ambush) && stats.containsKey(MobStat.PAD))) {
                return false;
            }
        }
        if (poison) {
            ...
            if (isBuffed(MobStat.Poison)) {
                stats.get(MobStat.Poison).nOption = (short) poisonDamage;
            }
            ...
            if (from.getJob() == MapleJob.NIGHTLORD || from.getJob() == MapleJob.SHADOWER || from.getJob().isA(MapleJob.NIGHTWALKER3)) {
                ...
                if (isBuffed(MobStat.Venom)) {
                    stats.get(MobStat.Venom).nOption = (short) poisonDamage;
                }
                if (isBuffed(MobStat.Poison)) {
                    stats.get(MobStat.Poison).nOption = (short) poisonDamage;
                }
            ...
            if (isBuffed(MobStat.Web)) {
                stats.get(MobStat.Web).nOption = (short) poisonDamage;
            }
            lastPoisonDamage = System.currentTimeMillis() + 1000;
            poisonType = 1;
            poisonFrom = from.getId();
            ...
            if (isBuffed(MobStat.Ambush)) {
                stats.get(MobStat.Ambush).nOption = (short) poisonDamage;
            }
        ...
        // map.announce(packet, getPosition());
    ...
    public void removeMobStats(Map<MobStat, MobStatData> stats) {
        ...
        map.announce(MobPool.StatReset(this, stats));
    }
    ...
    public void registerMobStats(Map<MobStat, MobStatData> stats) {
        ...
        if (map != null && !stats.isEmpty()) {
            map.announce(MobPool.StatSet(this, stats));
        }
    }
