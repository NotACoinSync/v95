...
public class ItemData {
    ...
    public void loadFromXML(String wzPath, int itemid) {
        ...
        for (MapleData data : itemData.getChildren()) {
            ...
            if (weapon_type != null) {
                weaponTypes.add(weapon_type);
            }
        }
        MapleData spec = itemData.getChildByPath("spec");
        if (spec != null) {
            itemEffect = MapleStatEffect.loadItemEffectFromData(spec, itemid, 1);
            ...
            for (MapleData infoSub : info.getChildren()) {
                switch (infoSub.getName()) {
                    ...
                    case "tuc":
                        ...
                        if (tuc > 0 && ItemConstants.MAX_SCROLLS != -1) {
                            tuc = ItemConstants.MAX_SCROLLS;
                        }
                        break;
                    case "cursed":
    ...
    public void saveToBin() {
        ...
        try {
            ...
            oPacket.writeDouble(unitPrice);
            oPacket.writeInt(recoveryHP);
            oPacket.writeInt(recoveryMP);
            oPacket.writeDouble(recovery);
            oPacket.writeInt(tamingMob);
            oPacket.writeInt(levelData.size());
            for (String level : levelData.keySet()) {
                oPacket.writeMapleAsciiString(level);
                levelData.get(level).save(oPacket);
            }
            oPacket.writeBoolean(isCash);
            oPacket.writeInt(meso);
            oPacket.writeInt(wholePrice);
            oPacket.writeMapleAsciiString(islot);
            oPacket.writeInt(reqJob);
            oPacket.writeInt(reqLevel);
            oPacket.writeInt(reqStr);
            oPacket.writeInt(reqDex);
            oPacket.writeInt(reqInt);
            oPacket.writeInt(reqLuk);
            oPacket.writeInt(reqPop);
            oPacket.writeShort(incLEV);
            oPacket.writeShort(incReqLevel);
            oPacket.writeShort(incStr);
            oPacket.writeShort(incInt);
            oPacket.writeShort(incDex);
            oPacket.writeShort(incLuk);
            oPacket.writeShort(incAcc);
            oPacket.writeShort(incPAD);
            oPacket.writeShort(incPDD);
            oPacket.writeShort(incMAD);
            oPacket.writeShort(incMDD);
            oPacket.writeShort(incHP);
            oPacket.writeShort(incMHP);
            oPacket.writeShort(incMaxHP);
            oPacket.writeShort(incMHPr);
            oPacket.writeShort(incMMP);
            oPacket.writeShort(incMaxMP);
            oPacket.writeShort(incMMPr);
            oPacket.writeShort(incEVA);
            oPacket.writeShort(incSpeed);
            oPacket.writeShort(incJump);
            oPacket.writeShort(incPVPDamage);
            oPacket.writeShort(incCraft);
            oPacket.writeInt(tuc);
            oPacket.writeInt(cursed);
            oPacket.writeInt(success);
            oPacket.writeInt(fs);
            oPacket.writeInt(rcount);
            oPacket.writeBoolean(tradeBlock);
            oPacket.writeBoolean(accountSharable);
            oPacket.writeInt(quest);
            oPacket.writeBoolean(equipTradeBlock);
            oPacket.writeBoolean(noCancelMouse);
            oPacket.writeBoolean(tradeAvailable);
            oPacket.writeBoolean(pickupRestricted);
            oPacket.writeMapleAsciiString(name);
            oPacket.writeMapleAsciiString(msg);
            oPacket.writeBoolean(onlyOnePickup);
            oPacket.writeInt(stateChangeItem);
            oPacket.writeInt(exp);
            oPacket.writeInt(maxLevel);
            oPacket.writeBoolean(consumeOnPickup);
            oPacket.writeInt(scriptedItem.npc);
            oPacket.writeMapleAsciiString(scriptedItem.script);
            oPacket.writeBoolean(scriptedItem.runOnPickup);
            oPacket.writeInt(attackSpeed);
            oPacket.writeShort(lv);
            oPacket.writeInt(randOption);
            oPacket.writeInt(randStat);
            oPacket.writeInt(slotMax);
            oPacket.writeBoolean(rewardItems != null);
            if (rewardItems != null) {
                oPacket.writeInt(rewardItems.left);
                oPacket.writeInt(rewardItems.right.size());
                for (RewardItem item : rewardItems.right) {
                    item.save(oPacket);
                }
            }
            oPacket.writeBoolean(itemEffect != null);
            if (itemEffect != null) {
                itemEffect.save(oPacket);
            }
            oPacket.writeInt(petCanConsume.size());
            for (int id : petCanConsume) {
                oPacket.writeInt(id);
            }
            oPacket.writeInt(masterLevel);
            oPacket.writeInt(reqSkillLevel);
            oPacket.writeInt(skills.size());
            for (int skill : skills) {
                oPacket.writeInt(skill);
            }
            oPacket.writeInt(mobs.size());
            for (Pair<Integer, Integer> p : mobs) {
                oPacket.writeInt(p.left);
                oPacket.writeInt(p.right);
            }
            oPacket.writeInt(allowedItems.size());
            for (int item : allowedItems) {
                oPacket.writeInt(item);
            }
            oPacket.writeInt(chatBalloon);
            oPacket.writeInt(nameTag);
            oPacket.writeInt(hungry);
            oPacket.writeInt(life);
            oPacket.writeInt(petData.size());
            for (String skillid : petData.keySet()) {
                oPacket.writeMapleAsciiString(skillid);
                petData.get(skillid).save(oPacket);
            }
            oPacket.writeInt(skillData.size());
            for (int level : skillData.keySet()) {
                oPacket.writeInt(level);
                skillData.get(level).save(oPacket);
            }
            oPacket.writeInt(probForLevelSkill);
            oPacket.writeInt(statIncreaseProb);
            oPacket.writeBoolean(noRevive);
            oPacket.writeInt(limitedLife);
            oPacket.writeBoolean(noMoveToLocker);
            oPacket.writeInt(absAction.size());
            for (String s : absAction) {
                oPacket.writeMapleAsciiString(s);
            }
            oPacket.writeBoolean(notSale);
            oPacket.writeBoolean(expireOnLogout);
            oPacket.writeInt(weaponTypes.size());
            for (int type : weaponTypes) {
                oPacket.writeInt(type);
            }
            oPacket.writeInt(maplepoint);
            oPacket.writeInt(rate);
            // time shit
            oPacket.writeInt(protectTime);
            oPacket.writeInt(karma);
            oPacket.writeInt(npc);
            oPacket.writeInt(recoveryRate);
            oPacket.writeBoolean(monsterBook);
            oPacket.writeInt(mob);
            oPacket.writeInt(max);
            oPacket.writeBoolean(mobPotion);
            oPacket.writeInt(mobHp);
            oPacket.writeInt(mobID);
            oPacket.writeInt(durability);
            oPacket.writeBoolean(evol);
            oPacket.writeBoolean(autoReact);
            oPacket.writeInt(evol1);
            oPacket.writeInt(evol2);
            oPacket.writeInt(evol3);
            oPacket.writeInt(evol4);
            oPacket.writeInt(evol5);
            oPacket.writeInt(evolNo);
            oPacket.writeInt(evolProb1);
            oPacket.writeInt(evolProb2);
            oPacket.writeInt(evolProb3);
            oPacket.writeInt(evolProb4);
            oPacket.writeInt(evolProb5);
            oPacket.writeInt(evolReqItemID);
            oPacket.writeInt(evolReqPetLvl);
            oPacket.writeBoolean(permanent);
            oPacket.writeBoolean(sweepForDrop);
            oPacket.writeBoolean(interactByUserAction);
            oPacket.writeBoolean(timeLimited);
            oPacket.writeBoolean(preventslip);
            oPacket.writeBoolean(warmsupport);
            oPacket.writeBoolean(recover);
            oPacket.writeBoolean(randstat);
            oPacket.saveToFile(file);// 24
        } catch (Exception ex) {
    ...
    public static class LevelData {
        ...
        public void save(MaplePacketLittleEndianWriter oPacket) {
            oPacket.writeInt(incDEXMin);
            oPacket.writeInt(incDEXMax);
            oPacket.writeInt(incSTRMin);
            oPacket.writeInt(incSTRMax);
            oPacket.writeInt(incINTMin);
            oPacket.writeInt(incINTMax);
            oPacket.writeInt(incLUKMin);
            oPacket.writeInt(incLUKMax);
            oPacket.writeInt(incMHPMin);
            oPacket.writeInt(incMHPMax);
            oPacket.writeInt(incMMPMin);
            oPacket.writeInt(incMMPMax);
            oPacket.writeInt(incPADMin);
            oPacket.writeInt(incPADMax);
            oPacket.writeInt(incMADMin);
            oPacket.writeInt(incMADMax);
            oPacket.writeInt(incPDDMin);
            oPacket.writeInt(incPDDMax);
            oPacket.writeInt(incMDDMin);
            oPacket.writeInt(incMDDMax);
            oPacket.writeInt(incACCMin);
            oPacket.writeInt(incACCMax);
            oPacket.writeInt(incEVAMin);
            oPacket.writeInt(incEVAMax);
            oPacket.writeInt(incSpeedMin);
            oPacket.writeInt(incSpeedMax);
            oPacket.writeInt(incJumpMin);
            oPacket.writeInt(incJumpMax);
            oPacket.writeInt(exp);
            oPacket.writeDouble(statIncreaseProb);
        }
    }
}
