...
public class MapleMapData {
    ...
    public void save(MaplePacketLittleEndianWriter oPacket) {
        oPacket.writeMapleAsciiString(onFirstUserEnter);
        oPacket.writeMapleAsciiString(onUserEnter);
        oPacket.writeInt(returnMap);
        oPacket.writeInt(forcedReturnMap);
        oPacket.writeInt(fieldLimit);
        oPacket.writeInt(VRTop);
        oPacket.writeInt(VRBottom);
        oPacket.writeInt(VRLeft);
        oPacket.writeInt(VRRight);
        oPacket.writeShort(mobInterval);
        oPacket.write(monsterRate);
        oPacket.writeInt(mobCapacity);
        oPacket.writeInt(fieldType);
        oPacket.writeInt(town);
        oPacket.writeInt(decHP);
        oPacket.writeInt(protectItem);
        //
        oPacket.writeLong(timeLimit);
        //
        oPacket.writeMapleAsciiString(mapName);
        oPacket.writeMapleAsciiString(streetName);
        oPacket.writeMapleAsciiString(mapMark);
        //
        oPacket.writeBoolean(everlast);
        oPacket.writeBoolean(clock);
        oPacket.writeBoolean(boat);
        //
        footholds.save(oPacket);// todo:
        oPacket.writeInt(ladderFootholds.size());
        for (MapleLadderFoothold lfh : ladderFootholds) {
            lfh.save(oPacket);
        }
        oPacket.writeBoolean(timeMob != null);
        if (timeMob != null) {
            oPacket.writeInt(timeMob.left);
            oPacket.writeMapleAsciiString(timeMob.right);
        }
        oPacket.writeInt(areas.size());
        for (Rectangle rect : areas) {
            oPacket.writeInt(rect.x);
            oPacket.writeInt(rect.y);
            oPacket.writeInt(rect.width);
            oPacket.writeInt(rect.height);
        }
        oPacket.writeInt(monsterSpawn.size());
        for (SpawnPoint sp : monsterSpawn) {
            sp.save(oPacket);
        }
        oPacket.writeInt(customMonsterSpawn.size());
        for (SpawnPoint sp : customMonsterSpawn) {
            sp.save(oPacket);
        }
        oPacket.writeInt(mapObjects.size());
        for (MapleMapObject mo : mapObjects) {
            oPacket.writeBoolean((mo instanceof MapleNPC));
            if (mo instanceof MapleNPC) {
                ((MapleNPC) mo).save(oPacket);
            } else if (mo instanceof PlayerNPC) {
                ((PlayerNPC) mo).save(oPacket);
            } else {
                System.out.println("Bad mmo in MapleMapData: " + mo.getType().name());
            }
        }
        oPacket.writeInt(reactors.size());
        for (MapleReactor mr : reactors) {
            saveReactor(mr, oPacket);
        }
        oPacket.writeInt(portals.size());
        for (MaplePortal mp : portals) {
            mp.save(oPacket);
        }
        oPacket.writeInt(backgroundTypes.size());
        for (int in : backgroundTypes.keySet()) {
            oPacket.writeInt(in);
            oPacket.writeInt(backgroundTypes.get(in));
        }
        // mcs
        oPacket.writeMapleAsciiString(bgm);
        oPacket.writeBoolean(swim);
        oPacket.writeBoolean(fly);
    ...
    private MapleReactor loadReactor(LittleEndianAccessor lea) {
        ...
        if (lea.readBoolean()) {
            myReactor.setName(lea.readMapleAsciiString());
        } else {
            myReactor.setName("");
        }
        return myReactor;
    ...
    private void saveReactor(MapleReactor mr, LittleEndianWriter lew) {
        ...
        if (mr.getName() != null) {
            lew.writeMapleAsciiString(mr.getName());
        }
    }
