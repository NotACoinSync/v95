...
public class MapleFootholdTree {
    ...
    public void save(MaplePacketLittleEndianWriter oPacket) {
        oPacket.writeBoolean(nw != null);
        if (nw != null) {
            nw.save(oPacket);
        }
        oPacket.writeBoolean(ne != null);
        if (ne != null) {
            ne.save(oPacket);
        }
        oPacket.writeBoolean(sw != null);
        if (sw != null) {
            sw.save(oPacket);
        }
        oPacket.writeBoolean(se != null);
        if (se != null) {
            se.save(oPacket);
        }
        oPacket.writeInt(footholds.size());
        for (MapleFoothold fh : footholds) {
            fh.save(oPacket);
        }
        oPacket.writePos(p1);
        oPacket.writePos(p2);
        oPacket.writePos(center);
        oPacket.writeInt(depth);
        oPacket.writeInt(minDropX);
        oPacket.writeInt(maxDropX);
    }
