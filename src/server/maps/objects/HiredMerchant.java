...
public class HiredMerchant extends PlayerShop {
    ...
    public void addVisitor(MapleCharacter visitor) {
        ...
        if (i > -1) {
            ...
            // }else{
            visitors[i] = visitor;
            broadcastToVisitors(MiniRoomBaseDlg.EntrustedShop.hiredMerchantVisitorAdd(visitor, i + 1));
            // }
        }
    }
    ...
    public void removeVisitor(MapleCharacter visitor) {
        ...
        if (visitors[slot] != null && visitors[slot].getId() == visitor.getId()) {
            visitors[slot] = null;
            if (slot != -1) {
                broadcastToVisitors(MiniRoomBaseDlg.EntrustedShop.hiredMerchantVisitorLeave(slot + 1));
            }
        }
    }
    ...
    public void removeAllVisitors(String message) {
        for (int i = 0; i < 3; i++) {
            if (visitors[i] != null) {
                ...
                visitors[i].getClient().announce(MiniRoomBaseDlg.EntrustedShop.leaveHiredMerchant(i + 1, 0x11));
                if (message.length() > 0) {
    ...
    public void forceClose() {
        ...
        // Server.getInstance().getChannel(world, channel).removeHiredMerchant(ownerId);
        map.announce(EmployeePool.EmployeeLeaveField(getOwnerId()));
    ...
    public void closeShop(MapleClient c, boolean timeout) {
        ...
        map.removeMapObject(this);
        map.announce(EmployeePool.EmployeeLeaveField(ownerId));
    ...
    public void sendSpawnData(MapleClient client) {
        client.announce(EmployeePool.EmployeeEnterField(this));
    }
