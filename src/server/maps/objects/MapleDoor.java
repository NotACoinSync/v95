...
public class MapleDoor extends AbstractMapleMapObject implements Externalizable {
    ...
    public MapleDoor(MapleCharacter owner, int nSkillID, Point targetPosition) {
        ...
        if (townPortal != null) {
            this.townPortalName = townPortal.getName();
        }
    }
    ...
    public void sendSpawnData(MapleClient client) {
        ...
        if (client.getChannel() == channel && owner != null) {
            ...
            client.announce(TownPortalPool.Created(getOwner(), getTown().getId() == client.getPlayer().getMapId() ? getTownPortal().getPosition() : targetPosition, getTown().getId() == client.getPlayer().getMapId()));
            // spawns the portals to actually warp.
            client.announce(WvsContext.TownPortal.Created(getTown().getId(), getTarget().getId(), nSkillID, targetPosition));
            if (owner.isInParty()) {
    ...
    public void sendDestroyData(MapleClient client) {
        ...
        if (client.getChannel() == channel && owner != null) {
            ...
            client.announce(TownPortalPool.Removed(getOwner()));
            client.announce(WvsContext.TownPortal.Removed(getTargetPosition()));
            // }
        }
    }
    ...
    public MapleMap getTown() {
        if (town == null) {
            town = ChannelServer.getInstance().getMap(channel, townid);
        }
        return town;
    }
    ...
    public MaplePortal getTownPortal() {
        if (townPortal == null) {
            townPortal = getTown().getPortal(townPortalName);
        }
        return townPortal;
    }
    ...
    public MapleMap getTarget() {
        if (target == null) {
            target = ChannelServer.getInstance().getMap(channel, targetid);
        }
        return target;
    }
