...
public class SharkPacket {
    ...
    public void dump(MaplePacketLittleEndianWriter oPacket) {
        if (invalid) {
            return;
        }
        ...
        oPacket.writeLong(timestamp);
        oPacket.writeShort(size);
        oPacket.writeShort(opcode);
        oPacket.write(Arrays.copyOfRange(info, 2, info.length));
    }
}
