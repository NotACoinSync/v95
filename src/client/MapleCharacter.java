...
import client.player.CharacterStat;
import client.player.CharacterStats;
import client.player.SecondaryStat;
import client.player.SecondaryStatHandler;
...
import tools.packets.CField.ScriptMan;
...
public class MapleCharacter extends AbstractAnimatedMapleMapObject {
    ...
    protected static final String[] BLOCKED_NAMES = {"admin", "owner", "moderator", "intern", "donor", "administrator", "help", "helper", "alert", "notice", "maplestory",
        "Vertisy", "wizet", "security", "official", "support", "gamemaster", "gm", "operate", "master", "GameMaster", "community", "message", "event", "test", "Obama",
        "Suushi", "Tiger", "FangBlade", "ZeroByDivide", "Misusing", "lnmateSearch", "MapleRoyals", "Extalia", "MapleLegends", "HawtMaple", "MapleDestiny", "FluffyMS"};
    private int world;
    ...
    private EnumMap<SecondaryStat, MapleBuffStatDataHolder> effects = new EnumMap<>(SecondaryStat.class);
    ...
    //
    public SecondaryStatHandler secondaryStat;
    //
    ...
    //
    private List<Integer> capturedMob = new ArrayList<>();
    //
    public int predictQuit;
    ...
    protected MapleCharacter() {
        ...
        secondaryStat = new SecondaryStatHandler();
    ...
    public void update() {
        ...
        synchronized (effects) {
            for (Entry<SecondaryStat, MapleBuffStatDataHolder> entry : this.effects.entrySet()) {
                ...
                if (effect.isRecovery()) {
                    if (updateTime - lastRecoveryTime >= 5000L) {
                        addHP(effect.getX());
                        client.announce(EffectPacket.Local.IncDecHPEffect((byte) effect.getX()));
                        getMap().announce(this, EffectPacket.Remote.IncDecHPEffect(id, (byte) effect.getX()), false);
    ...
    public void checkBerserk() {
        if (job.equals(MapleJob.DARKKNIGHT)) {
            ...
            if (skilllevel > 0) {
                ...
                client.announce(EffectPacket.Local.SkillUse.LoadDarkForceEffect(getLevel(), skilllevel, berserk));
                getMap().announce(this, EffectPacket.Remote.SkillUse.LoadDarkForceEffect(getId(), getLevel(), skilllevel, berserk), false);
            }
        }
    }
    ...
    public void cancelBuffStats(SecondaryStat stat) {
        List<SecondaryStat> buffStatList = Arrays.asList(stat);
    ...
    public void setCombo(short count) {
        if (count < combocounter) {
            cancelEffectFromSecondaryStat(SecondaryStat.ComboAbilityBuff);
        }
        combocounter = (short) Math.min(30000, count);
        if (count > 0) {
            announce(UserLocal.IncComboResponse(combocounter));
        }
    }
    ...
    public static class CancelCooldownAction implements Runnable {
        ...
        public void run() {
            ...
            if (realTarget != null) {
                ...
                realTarget.client.announce(PacketHelper.skillCooldown(skillId, 0));
            }
        }
    }
    ...
    public void cancelEffect(MapleStatEffect effect, boolean overwrite, long startTime) {
        ...
        if (!overwrite) {
            ...
            List<Pair<SecondaryStat, BuffDataHolder>> statups = effect.getStatups();
        ...
        if (effect.getSourceId() >= 2022125 && effect.getSourceId() <= 2022129) {// beholder
            for (SecondaryStat stat : effects.keySet()) {
        ...
        if (effect.getSourceId() == Spearman.HYPER_BODY || effect.getSourceId() == SuperGM.HyperBody) {
            ...
            client.announce(WvsContext.StatChanged(statup, this));
        }
    ...
    public void cancelEffectFromSecondaryStat(SecondaryStat stat) {
    ...
    public void Hide(boolean hide, boolean login) {
        if (isGM() && hide != this.hidden) {
            if (!hide) {
                this.hidden = false;
                announce(Field.AdminResult.getGMEffect(AdminResult.HIDE, (byte) 0));
                List<SecondaryStat> dsstat = Collections.singletonList(SecondaryStat.DarkSight);
                getMap().announceGM(this, UserRemote.resetTemporaryStat(id, dsstat), false);
                getMap().announce(this, UserPool.UserEnterField(this), false);
                updatePartyCharacter();
            } else {
                this.hidden = true;
                announce(Field.AdminResult.getGMEffect(AdminResult.HIDE, (byte) 1));
                if (!login) {
                    getMap().announce(this, UserPool.UserLeaveField(getId()), false);
                    getMap().announceGM(this, UserPool.UserEnterField(this), false);
                    ...
                    getMap().announceGM(this, UserRemote.setTemporaryStat(this, dsstat), false);
                }
            ...
            if (hidden) {
                if (uiToggle) {
                    dropMessage(MessageType.TITLE, "You are now hidden");
                }
                dropMessage(MessageType.SYSTEM, "You are now hidden");
            } else {
                if (uiToggle) {
                    dropMessage(MessageType.TITLE, "You are now visible");
                }
    ...
    private void cancelPlayerBuffs(List<SecondaryStat> buffstats) {
        if (client.getChannelServer().getPlayerStorage().getCharacterById(getId()) != null) {
            // dropMessage("recalcLocalStats() in cancelPlayerBuffs()");
            recalcLocalStats();
            enforceMaxHpMp();
            client.announce(WvsContext.resetTemporaryStat(buffstats));
            if (buffstats.size() > 0) {
                getMap().announce(this, UserRemote.resetTemporaryStat(getId(), buffstats), false);
            }
        }
    }
    ...
    public static boolean canCreateChar(String name) {
        for (String nameTest : BLOCKED_NAMES) {
            if (name.toLowerCase().contains(nameTest.toLowerCase())) {
                return false;
            }
        }
    ...
    public void setMasteries(int jobId) {
        ...
        switch (jobId) {
            ...
            case 322:
                ...
                break;
            case 412:
                skills[0] = NightLord.ShadowStars;
    ...
    public void changeJob(MapleJob newJob) {
        ...
        if (this.guildid > 0) {
            try {
                ChannelServer.getInstance().getWorldInterface().guildMessage(guildid, WvsContext.NotifyJobChange(0, job.getId(), name), this.getId());
            } catch (RemoteException | NullPointerException ex) {
                Logger.log(LogType.ERROR, LogFile.REMOTE_EXCEPTION, ex);
            }
        }
        ...
        guildUpdate();
        getMap().announce(this, EffectPacket.Remote.JobChanged(getId()), false);
        if (getJob().isA(MapleJob.EVAN1)) {
            if (getBuffedValue(SecondaryStat.RideVehicle) != null) {
                cancelBuffStats(SecondaryStat.RideVehicle);
            }
            createDragon();
        }
    }
    ...
    public void warpToPortal(byte portalid) {
        announce(UserLocal.Teleport(portalid, 1));
    }
    ...
    public void changeMap(final MapleMap to, final Point pos) {
        changeMapInternal(to, pos, Stage.SetField(to, (byte) 0x80, null, this, false));
    }

    public void changeMapPosition(final MapleMap to, final Point pos) {
        changeMapInternal(to, pos, Stage.SetField(to, (byte) 0x80, pos, this, false));
    }
    ...
    public void changeMap(final MapleMap to, MaplePortal pto) {
        ...
        changeMapInternal(to, pto.getPosition(), Stage.SetField(to, pto.getId(), null, this, false));
    }
    ...
    public void changeMapPortalPosition(final MapleMap to, MaplePortal pto) {
        ...
        changeMapInternal(to, pto.getPosition(), Stage.SetField(to, pto.getId(), pto.getPosition(), this, false));
    }
    ...
    private void changeMapInternal(MapleMap to, final Point pos, final byte[] warpPacket) {// test map logging
        ...
        if (to.getId() >= 109010000 && to.getId() <= 109010400 && getJewel() != null) {
            ...
            getMap().announce(Clock.Created((int) getJewel().getTimeLeft() / 1000, 2));
        }
        client.announce(warpPacket);
        if (this.getMarriedTo() > 0) {
            try {
                ChannelServer.getInstance().getWorldInterface().broadcastPacket(Arrays.asList(getMarriedTo()), WvsContext.NotifyWeddingPartnerTransfer(getMapId(), to.getId()));
            } catch (RemoteException | NullPointerException ex) {
                Logger.log(LogType.ERROR, LogFile.EXCEPTION, ex);
            }
        }
        ...
        if (client.getChannelServer().getPlayerStorage().getCharacterById(getId()) != null) {
            ...
            if (eventInstance != null) {
                ...
                if (clockTime > 0) {
                    announce(Clock.Created(clockTime, 2));
                } else {
                    announce(Clock.Destroy());
                }
            }

            if (ServerConstants.secondsLeft > 0) { // TODO: Chuyển sang MapleMap (addPlayer)
                map.announce(Clock.Created(ServerConstants.secondsLeft));
            } else if (ServerConstants.clockAll > 0) {
                map.announce(Clock.Created(ServerConstants.clockAll));
            }
            ...
            if (isInParty()) {
                ...
                client.announce(WvsContext.PartyResult.updateParty(client.getChannel(), getParty(), PartyOperation.SILENT_UPDATE, null));
                updatePartyCharacter();
            }
            getClient().announce(MobPool.MobCrcKeyChanged());
    ...
    public void changeSkillLevel(Skill skill, byte newLevel, int newMasterlevel, long expiration) {
        if (newLevel > -1) {
            ...
            if (!GameConstants.isHiddenSkills(skill.getId())) {
                this.client.announce(WvsContext.ChangeSkillRecordResult(skill.getId(), newLevel, newMasterlevel, expiration));
            }
        } else {
            skills.remove(skill);
            this.client.announce(WvsContext.ChangeSkillRecordResult(skill.getId(), newLevel, newMasterlevel, -1)); // Shouldn't use expiration anymore :)
            try {
                Connection con = DatabaseConnection.getConnection();
    ...
    public void decreaseBattleshipHp(int decrease) {
        ...
        if (battleshipHp <= 0) {
            ...
            announce(PacketHelper.skillCooldown(Corsair.BATTLE_SHIP, cooldown));
            ...
            cancelEffectFromSecondaryStat(SecondaryStat.RideVehicle);
        } else {
            announce(PacketHelper.skillCooldown(5221999, battleshipHp / 10)); // :D
    ...
    private void deregisterBuffStats(List<SecondaryStat> stats) {
        synchronized (stats) {
            for (SecondaryStat stat : stats) {
                if (stat.equals(SecondaryStat.Dash_Jump)) {
                ...
                } else if (stat.equals(SecondaryStat.Dash_Speed)) {
                ...
                MapleBuffStatDataHolder mbsvh = effects.get(stat);
                if (mbsvh != null) {
                    if (stat == SecondaryStat.ComboCounter) {
                        setBuffedValue(SecondaryStat.ComboCounter, 1);
                    }
                    effects.remove(stat);
                    if (stat == SecondaryStat.Regen) {
                        if (recoveryTask != null) {
                            recoveryTask.cancel(false);
                            recoveryTask = null;
                        }
                    } else if (stat == SecondaryStat.SUMMON || stat == SecondaryStat.PUPPET) {
                        int summonId = mbsvh.effect.getSourceId();
                        MapleSummon summon = summons.get(summonId);
                        if (summon != null) {
                            getMap().announce(SummonedPool.Removed(summon, true), summon.getPosition());
                    ...
                    } else if (stat == SecondaryStat.DragonBlood) {
    ...
    public final boolean hasDisease(final SecondaryStat dis) {
        for (final SecondaryStat disease : effects.keySet()) {
            if (disease == dis) {
                return true;
            }
        }
    ...
    public void dispelDebuffs() {
        List<SecondaryStat> toDispel = new ArrayList<>();
        toDispel.add(SecondaryStat.Curse);
        toDispel.add(SecondaryStat.Darkness);
        toDispel.add(SecondaryStat.Poison);
        toDispel.add(SecondaryStat.Seal);
        toDispel.add(SecondaryStat.Weakness);
    ...
    public void cancelAllDebuffs() {
        ...
        toDispel.add(SecondaryStat.Seal);
        toDispel.add(SecondaryStat.Weakness);
        toDispel.add(SecondaryStat.Attract);
        toDispel.add(SecondaryStat.ReverseInput);
        this.cancelPlayerBuffs(toDispel);
    }
    ...
    public void doHurtHp() {
        if (this.getInventory(MapleInventoryType.EQUIPPED).findById(getMap().getMapData().getHPDecProtect()) != null) {
            return;
        }
    ...
    public void dropMessage(MessageType type, String message) {
        switch (type) {
            case POPUP:
                client.announce(WvsContext.BroadcastMsg.encode(type.getValue(), message));
                client.announce(WvsContext.enableActions());
                break;
            case NOTICE:
            case MEGAPHONE:
            ...
            case MAPLETIP:
                ...
                break;
            case TITLE:
                client.announce(WvsContext.ScriptProgressMessage(message));
                break;
            default:
                client.announce(WvsContext.BroadcastMsg.encode(MessageType.SYSTEM.getValue(), message));
        }
    }
    ...
    private void enforceMaxHpMp() {
        ...
        if (stats.size() > 0) {
            client.announce(WvsContext.StatChanged(stats, this));
        }
    }
    ...
    public void equipChanged() {
        getMap().announce(this, UserRemote.avatarModified(this), false);
        recalcLocalStats();
        enforceMaxHpMp();
                        ...
                        for (Item item : inv.list()) {
                            ...
                            if ((expiration != -1 && expiration < currenttime) || (item.getPet() != null && item.getPet().getRemainLife() < 0)) {
                                if ((item.getPet() != null && item.getPet().getRemainLife() < 0) || item.getPet() == null) {
                                    client.announce(WvsContext.OnMessage.itemExpired(item.getItemId()));
                                    if (item.getPet() != null) {
    ...
    public void forceUpdateItem(Item item) {
        ...
        client.announce(WvsContext.InventoryOperation(true, mods));
    }
    ...
    public void gainExp(ExpProperty property) {
        ...
        if (this.getBuffedValue(SecondaryStat.Curse) != null) {
            property.gain *= 0.5;
            property.party *= 0.5;
        }
        if (pendantExp > 0) {
            property.equip((property.gain / 10) * pendantExp);
        }
        ...
        if (level < getMaxLevel()) {
            ...
            if (property.show && property.gain != 0) {
                client.announce(WvsContext.OnMessage.getShowExpGain(property));
            }
    ...
    public void gainMeso(int gain, boolean show, boolean enableActions, boolean inChat) {
        ...
        if (show) {
            client.announce(WvsContext.OnMessage.getShowMesoGain(gain, inChat));
        }
    }

    public void genericGuildMessage(int code) {
        this.client.announce(WvsContext.GuildResult.genericGuildMessage((byte) code));
    }
    ...
    public List<Pair<SecondaryStat, BuffDataHolder>> getAllStatups() {
        List<Pair<SecondaryStat, BuffDataHolder>> ret = new ArrayList<>();
        for (SecondaryStat mbs : effects.keySet()) {
    ...
    public MapleStatEffect getBuffEffect(SecondaryStat stat) {
        MapleBuffStatDataHolder mbsvh = effects.get(stat);
        if (mbsvh == null) {
            return null;
        } else {
    ...
    private List<SecondaryStat> getBuffStats(MapleStatEffect effect, long startTime) {
        List<SecondaryStat> stats = new ArrayList<>();
        for (Entry<SecondaryStat, MapleBuffStatDataHolder> stateffect : effects.entrySet()) {
            if (stateffect == null || stateffect.getValue() == null || stateffect.getValue().effect == null) {
                continue;
            }
    ...
    public void setSkillLevel(int skillId, byte level) {
    ...
    public MapleStatEffect getStatForBuff(SecondaryStat effect) {
        MapleBuffStatDataHolder mbsvh = effects.get(effect);
        if (mbsvh == null) {
            return null;
        }
    ...
    private void guildUpdate() {
        ...
        try {
            ...
            if (allianceId > 0) {
                ChannelServer.getInstance().getWorldInterface().allianceMessage(allianceId, WvsContext.AllianceResult.updateAllianceJobLevel(this), getId(), -1);
            }
        } catch (Exception e) {
            Logger.log(LogType.ERROR, LogFile.REMOTE_EXCEPTION, e);
        }
    ...
    public void handleEnergyChargeGain() { // to get here energychargelevel has to be > 0
        ...
        if (energybar < 10000) {
            ...
            List<Pair<SecondaryStat, BuffDataHolder>> stat = Collections.singletonList(new Pair<>(SecondaryStat.EnergyCharged, new BuffDataHolder(energycharge.getId(), getSkillLevel(energycharge), energybar)));
            setBuffedValue(SecondaryStat.EnergyCharged, energybar);
            client.announce(WvsContext.setTemporaryStat(this, energybar, 0, stat));
            client.announce(EffectPacket.Local.SkillSpecialAffected(energycharge.getId(), energycharge.getMaxLevel()));
            getMap().announce(this, EffectPacket.Remote.SkillSpecialAffected(id, energycharge.getId(), energycharge.getMaxLevel()));
            getMap().announce(this, UserRemote.setTemporaryStat(MapleCharacter.this, stat));
        }
        if (energybar >= 10000 && energybar < 11000) {
            ...
            tMan.schedule("energyBar", new Runnable() {
                ...
                public void run() {
                    energybar = 0;
                    List<Pair<SecondaryStat, BuffDataHolder>> stat = Collections.singletonList(new Pair<>(SecondaryStat.EnergyCharged, new BuffDataHolder(0, 0, energybar)));
                    setBuffedValue(SecondaryStat.EnergyCharged, energybar);
                    client.announce(WvsContext.setTemporaryStat(MapleCharacter.this, energybar, 0, stat));
                    getMap().announce(chr, UserRemote.setTemporaryStat(MapleCharacter.this, stat));
                    recalcLocalStats();
                }
            }, ceffect.getDuration());
            recalcLocalStats();
    ...
    public void handleOrbconsume() {
        ...
        List<Pair<SecondaryStat, BuffDataHolder>> stat = Collections.singletonList(new Pair<>(SecondaryStat.ComboCounter, new BuffDataHolder(skillid, getSkillLevel(combo), 1)));
        setBuffedValue(SecondaryStat.ComboCounter, 1);
        client.announce(WvsContext.setTemporaryStat(this, skillid, combo.getEffect(getSkillLevel(combo)).getDuration() + (int) ((getBuffedStarttime(SecondaryStat.ComboCounter) - System.currentTimeMillis())), stat));
        getMap().announce(this, UserRemote.setTemporaryStat(this, stat), false);
    }
    ...
    public void levelUp(boolean takeexp) {
        ...
        if (level == 10 && job.equals(MapleJob.EVAN)) {
            announce(ScriptMan.SayImage(NPCConversationManager.NpcReplacedByUser, 0, (byte) 0, "UI/tutorial/evan/14/0"));
            ...
            if (dragon != null) {
                ...
                if (isHidden()) {
                    getMap().broadcastGMMessage(this, DragonPacket.Created(dragon));
                } else {
                    getMap().announce(this, DragonPacket.Created(dragon));
                }
            }
        }
        if (job.isA(MapleJob.EVAN1)) {
            ...
            if (dragon != null) {
                ...
                if (level > 50) {
                    announce(WvsContext.InventoryOperation(true, Collections.singletonList(new ModifyInventory(3, dragon))));
                }
                announce(WvsContext.InventoryOperation(true, Collections.singletonList(new ModifyInventory(0, dragon))));
            }
        }
        announce(Field.FieldEffect.showEffect("LevelUp/BackGround"));
        announce(Field.FieldEffect.showEffect("LevelUp/Number/" + level));
        if (level >= getMaxLevel()) {
            exp.set(0);
        ...
        getMap().announce(this, EffectPacket.Remote.LevelUp(getId()), false);
        recalcLocalStats();
        ...
        if (this.guildid > 0 && newLevel) {
            ...
            try {
                ...
                ChannelServer.getInstance().getWorldInterface().guildMessage(guildid, WvsContext.NotifyLevelUp(2, level, name), getId());
            } catch (RemoteException | NullPointerException ex) {
                Logger.log(LogType.ERROR, LogFile.REMOTE_EXCEPTION, ex);
            }
        }
        if (level == 200 && !isGM() && newLevel) {
            ...
            try {
                ChannelServer.getInstance().getWorldInterface().broadcastPacket(WvsContext.BroadcastMsg.encode(6, String.format(LEVEL_200, names, names)));
            } catch (RemoteException | NullPointerException ex) {
                Logger.log(LogType.ERROR, LogFile.REMOTE_EXCEPTION, ex);
            }
        }
    ...
    public int getJobIndexBasedOnLevel(MapleJob job) {
        if (level <= 7 && JobConstants.is_beginner_job(job.getId())) {// beginner get its own sp
            ...
            if (level >= 160) {
                actualJob = MapleJob.EVAN10;
            } else if (level >= 120) {
                actualJob = MapleJob.EVAN9;
            } else if (level >= 100) {
                actualJob = MapleJob.EVAN8;
            } else if (level >= 80) {
                actualJob = MapleJob.EVAN7;
            } else if (level >= 60) {
                actualJob = MapleJob.EVAN6;
            } else if (level >= 50) {
                actualJob = MapleJob.EVAN5;
            } else if (level >= 40) {
                actualJob = MapleJob.EVAN4;
            } else if (level >= 30) {
                actualJob = MapleJob.EVAN3;
            } else if (level >= 20) {
                actualJob = MapleJob.EVAN2;
            } else if (level >= 10) {
                actualJob = MapleJob.EVAN1;
            } else {
                actualJob = MapleJob.EVAN;
            }
            return JobConstants.getJobIndex(actualJob.getId());
        }/*
         else if(JobConstants.is_dualblade_job(job.getId()) || this.nSubJob ==
         1){
         MapleJob actualJob = null;
    ...
    public void gainSp(int jobid, int amount) {
        ...
        client.announce(WvsContext.StatChanged(statup, this));
        client.announce(WvsContext.OnMessage.incSPMessage((short) jobid, (byte) amount));
    }
    ...
    public static MapleCharacter loadCharFromDB(int charid, MapleClient client, boolean channelserver) throws SQLException {
        try {
            ...
            for (RSSkill skill : RSSkill.values()) {
                if (!ret.rsSkillLevel.containsKey(skill)) {
                    ret.rsSkillLevel.put(skill, (byte) 1);
                }
            }
            ...
            for (RSSkill skill : RSSkill.values()) {
                if (!ret.rsSkillExp.containsKey(skill)) {
                    ret.rsSkillExp.put(skill, (long) 0);
                }
            }
            ...
            if (channelserver) {
                ...
                if (ret.map == null) {
                    ret.map = ret.client.getChannelServer().getMap(100000000);// rip
                }
    ...
    public void yellowMessage(String m) {
        announce(WvsContext.SetWeekEventMessage(m));
    }
    ...
    public void mobKilled(int id) {
        ...
        try {
            for (MapleQuestStatus q : quests.values()) {
                ...
                if (q.progress(id)) {
                    client.announce(WvsContext.OnMessage.updateQuest(q, 0));
                }
            }
        } catch (Exception e) {
    ...
    private void playerDead(MapleCharacter damager) {
        ...
        if (getBuffedValue(SecondaryStat.Morph) != null) {
            cancelEffectFromSecondaryStat(SecondaryStat.Morph);
        }
        if (getBuffedValue(SecondaryStat.RideVehicle) != null) {
            cancelEffectFromSecondaryStat(SecondaryStat.RideVehicle);
        }
        if (getChair() == -1) {
            setChair(0);
            client.announce(UserLocal.SitResult(-1));
            getMap().announce(this, UserRemote.setActivePortableChair(getId(), 0), false);
        }
        if (isHardMode()) {
            ...
            sendKeymap();
            sendMacros();
            announce(Stage.SetField(getMap(), (byte) 0, null, this, true));
            for (RSSkill skill : RSSkill.values()) {
    ...
    private void prepareDragonBlood(final MapleStatEffect bloodEffect) {
        ...
        dragonBloodSchedule = TimerManager.getInstance().register("dragonBlood", new Runnable() {
            ...
            public void run() {
                ...
                client.announce(EffectPacket.Local.SkillSpecial(bloodEffect.getSourceId(), 0, 0, 0));
                getMap().announce(MapleCharacter.this, EffectPacket.Remote.SkillSpecial(getId(), bloodEffect.getSourceId(), 0, 0, 0), false);
            }
        }, 4000, 4000);
    }
    ...
    public void recalcLocalStats() {
        ...
        for (Item item : getInventory(MapleInventoryType.EQUIPPED)) {
            ...
            if (equip.getDurability() == 0 && equip.getDurability() != -1) {
                continue;
            }
            ...
            if (skillData.getValue().skillevel > 0) {
                if (skillData.getKey().getId() % 10000 < 1000) {
                    ...
                    switch (skillData.getKey().getId()) {
                        ...
                        case Citizen.BlessingOfTheFairy:
                            watk += effect.getX();
                            magic += effect.getY();
                            break;
                    }
        ...
        Integer mwarr = getBuffedValue(SecondaryStat.BasicStatUp);
        if (mwarr != null) {
            STRr += mwarr;
            DEXr += mwarr;
            INTr += mwarr;
        ...
        Integer matkbuff = getBuffedValue(SecondaryStat.MAD);
        if (matkbuff != null) {
            magic += matkbuff;
        }
        Integer watkbuff = getBuffedValue(SecondaryStat.PAD);
        if (watkbuff != null) {
            watk += watkbuff;
        }
        Integer hbhp = getBuffedValue(SecondaryStat.MaxHP);
        if (hbhp != null) {
            localmaxhp += (hbhp.doubleValue() / 100) * localmaxhp;
        }
        Integer hbmp = getBuffedValue(SecondaryStat.MaxMP);
        if (hbmp != null) {
            localmaxmp += (hbmp.doubleValue() / 100) * localmaxmp;
        }
        MapleStatEffect combo = getBuffEffect(SecondaryStat.ComboAbilityBuff);
        if (combo != null) {
            watk += combo.getX();
        }
        if (energybar == 15000) {
        ...
        //
        Integer speedbuff = getBuffedValue(SecondaryStat.Speed);
        if (speedbuff != null) {
            // speed += speedbuff;
        }
        Integer jumpbuff = getBuffedValue(SecondaryStat.Jump);
        if (jumpbuff != null) {
            // jump += jumpbuff;
        }
        ...
        if (STRr > 0) {
            localstr += localstr * (STRr / 100);
        }
        if (DEXr > 0) {
            localdex += localdex * (DEXr / 100);
        }
        if (INTr > 0) {
            localint += localint * (INTr / 100);
        }
        if (LUKr > 0) {
            localluk += localluk * (LUKr / 100);
        }
        // Cap shit
        ...
        if (isInParty()) {
            ...
            for (MaplePartyCharacter partychar : getParty().getMembers()) {
                if (partychar.getMapId() == getMapId() && partychar.getChannel() == channel) {
                    client.announce(UserRemote.receiveHP(partychar.getId(), partychar.getHP(), partychar.getMaxHP()));
                }
            }
        }
    }
    ...
    /**
     Updates the MapleParty in the WorldServer with a completely updated
     MaplePartyCharacter. Does a silent update so it requires an upgraded
     grab.
     */
    public void updatePartyCharacter() {
        if (isInParty()) {
            ...
            try {
                ...
                // ChannelServer.getInstance().getWorldInterface().broadcastPacket(target, CWvsContext.PartyResult.updatePartyMemberHP(getId(), this.hp, this.localmaxhp));
            } catch (Exception ex) {
                Logger.log(LogType.ERROR, LogFile.REMOTE_EXCEPTION, ex);
                dropMessage(MessageType.ERROR, ServerConstants.WORLD_SERVER_ERROR);
            }
            /*
             ...
             try{
             ChannelServer.getInstance().getWorldInterface().broadcastPacket(target,
             CWvsContext.PartyResult.updatePartyMemberHP(getId(), this.hp,
             this.localmaxhp));
             }catch(Exception ex){
             Logger.log(LogType.ERROR, LogFile.REMOTE_EXCEPTION, ex);
             }
    ...
    public void registerEffect(MapleStatEffect effect, long starttime, long duration) {
        ...
        for (Pair<SecondaryStat, BuffDataHolder> statup : effect.getStatups()) {
    ...
    public void removeAllCooldownsExcept(int id, boolean packet) {
        for (MapleCoolDownValueHolder mcvh : coolDowns.values()) {
            if (mcvh.skillId != id) {
                ...
                if (packet) {
                    client.announce(PacketHelper.skillCooldown(mcvh.skillId, 0));
                }
            }
        }
    }
    ...
    public void resetStats() {
        ...
        announce(WvsContext.StatChanged(statup, this));
    }
    ...
    public void sendPolice(int greason, String reason, int duration) {
        announce(WvsContext.DataCRCCheckFailed(String.format("You have been blocked by the#b %s Police for %s.#k", "MèoStory", reason)));
        this.isbanned = true;
    ...
    public void sendPolice(String text) {
        ...
        try {
            ChannelServer.getInstance().getWorldInterface().broadcastGMPacket(WvsContext.SetWeekEventMessage(message));
        } catch (RemoteException | NullPointerException ex) {
            Logger.log(LogType.ERROR, LogFile.REMOTE_EXCEPTION, ex);
        }
        ...
        // }
        // Server.getInstance().announceGM(0, CWvsContext.BroadcastMsg.encode(1, getName() + " received this - " + text));
    ...
    public void sendKeymap() {
        client.announce(FuncKeyMappedMan.Init(keymap));
    }

    public void sendMacros() {
        // Always send the macro packet to fix a client side bug when switching characters.
        client.announce(WvsContext.getMacros(skillMacros));
    }
    ...
    public void setBuddyCapacity(int capacity) {
        ...
        client.announce(WvsContext.FriendResult.updateBuddyCapacity(capacity));
    }
    ...
    public boolean gainSlots(int type, int slots, boolean update) {
        if (inventory[type].getSlotLimit() < 96) {
            ...
            saveToDB();
            if (update) {
                client.announce(WvsContext.InventoryGrow(type, slots));
            }
            return true;
        }
        return false;
    ...
    public void showDojoClock() {
        ...
        if (stage % 6 > 0) {
            client.announce(Clock.Created((int) time, Clock.Dojang));
        }
        ...
        try {
            try (PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT * FROM notes WHERE `to`=? AND `deleted` = 0", ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    ...
                    int count = rs.getRow();
                    rs.first();
                    client.announce(WvsContext.MemoResult.Show(rs, count));
                }
            }
        } catch (SQLException e) {
            Logger.log(LogType.ERROR, LogFile.EXCEPTION, e);
    ...
    public void silentGiveBuffs(List<PlayerBuffValueHolder> buffs) {
        for (PlayerBuffValueHolder mbsvh : buffs) {
            if (mbsvh.sourceid != Priest.MysticDoor) {
                mbsvh.getEffect().silentApplyBuff(this, mbsvh.startTime, mbsvh.duration);
            }
        }
    }
    ...
    public void startFullnessSchedule(final int decrease, final MaplePet pet, final int petSlot) {
        ...
        ScheduledFuture<?> schedule = TimerManager.getInstance().register("startFullnessSchedule", new Runnable() {
            ...
            @Override
            public void run() {
                if (getCashShop().isOpened() || getClient().getPlayer() == null || getClient().getPlayer().getMap() == null) {
                    return;
                }
                    ...
                    if (newFullness <= 50) {
                        if (getClient().checkEliteStatus()) {
                            if (haveItem(2120000)) {
                                ...
                                fed = true;
                                getMap().announce(getClient().getPlayer(), PetPacket.petChat(getId(), (byte) petSlot, 0, 0, "Pet Food x" + getItemQuantity(2120000, false)), true);
                            }
                        }
                    }
                    if (!fed) {
    ...
    public void unequipPet(MaplePet pet, boolean shift_left, boolean hunger) {
        ...
        getMap().announce(this, PetPacket.showPet(this, pet, true, hunger), true);
    ...
    public void updateQuestInfo(int quest, String info) {
        ...
        announce(WvsContext.OnMessage.updateQuest(qs, 0));
        if (qs.getQuest().startQuestData.infoNumber > 0) {
            announce(WvsContext.OnMessage.updateQuest(qs, qs.getQuest().startQuestData.infoNumber));
        }
        if (qs.getStatus() == Status.STARTED) {
            if (qs.getQuest().completeQuestData.infoNumber > 0) {
                announce(WvsContext.OnMessage.updateQuest(qs, qs.getQuest().completeQuestData.infoNumber));
            }
        }
    ...
    public void updateQuest(MapleQuestStatus quest, boolean infoOnly, boolean info) {
        ...
        switch (quest.getStatus()) {
            case STARTED:
                announce(WvsContext.OnMessage.updateQuest(quest, 0));
                if (info) {
                    if (quest.getQuest().startQuestData.infoNumber > 0) {
                        if (quest.getQuestID() == quest.getQuest().startQuestData.infoNumber) {
                            ...
                            updateQuest(this.getQuest(MapleQuest.getInstance(quest.getQuest().startQuestData.infoNumber)), true, true);
                        }
                    }
                }
                if (!infoOnly) {
                    announce(UserLocal.QuestResult.updateQuestInfo((short) quest.getQuest().getId(), quest.getNpc()));
                }
                break;
            case COMPLETED:
                announce(WvsContext.OnMessage.completeQuest((short) quest.getQuest().getId(), quest.getCompletionTime()));
                announce(EffectPacket.Local.QuestComplete());
                getMap().announce(EffectPacket.Remote.QuestComplete(getId()));
                if (info) {
                    if (quest.getQuest().completeQuestData.infoNumber > 0) {
                        if (quest.getQuestID() == quest.getQuest().completeQuestData.infoNumber) {
                            ...
                            updateQuest(this.getQuest(MapleQuest.getInstance(quest.getQuest().completeQuestData.infoNumber)), true, true);
                        }
                    }
                }
                break;
    ...
    public void questTimeLimit(final MapleQuest quest, int time) {
        ...
        timers.put("" + quest.getId(), TimerManager.getInstance().schedule("questTimeLimit", new Runnable() {
            ...
            @Override
            public void run() {
                announce(UserLocal.QuestResult.questExpired(quest.getId()));
                MapleQuestStatus newStatus = new MapleQuestStatus(quest, MapleQuestStatus.Status.NOT_STARTED);
                newStatus.setForfeited(getQuest(quest).getForfeited() + 1);
                updateQuest(newStatus);
            }
        }, time * 60 * 1000));
        announce(UserLocal.QuestResult.addQuestTimer(quest.getId(), time * 60 * 1000));
    }

    // Wild Hunter Packets cần cái này
    public int getIntNRecord(int questID) {
        final MapleQuestStatus stat = getQuestNAdd((short) questID);
    ...
    private void updateSingleStat(MapleStat stat, int newValue, boolean itemReaction) {
        announce(WvsContext.StatChanged(Collections.singletonList(new Pair<>(stat, newValue)), itemReaction, this));
    }
    ...
    public void sendDestroyData(MapleClient client) {
        client.announce(UserPool.UserLeaveField(this.getObjectId()));
    }

    @Override
    public void sendSpawnData(MapleClient client) {
        if (!this.isHidden() || client.getPlayer().isGM()) {
            client.announce(UserPool.UserEnterField(this));
            if (driver != -1) {
                client.announce(UserCommon.followCharacter(id, driver));
            }
            if (passenger != -1) {
                client.announce(UserCommon.followCharacter(passenger, id));
            }
            if (getMount() != null && getMount().isActive()) {
                client.announce(UserRemote.showMonsterRiding(getId(), getMount()));
            }
        }
        if (this.isHidden()) {
    ...
    public void updateAreaInfo(int area, String info) {
        ...
        announce(WvsContext.OnMessage.updateAreaInfo(area, info));
    }
    ...
    public void autoban(String reason, AutobanFactory fac) {
        ...
        announce(WvsContext.DataCRCCheckFailed(String.format("You have been blocked from#b %s #k for #b" + fac.name() + ".#k", "MèoStory")));
        if (!isGM()) {
            client.disconnect(false, getCashShop().isOpened());
        }
        /*
         ...
         */
        try {
            ChannelServer.getInstance().getWorldInterface().broadcastGMPacket(WvsContext.BroadcastMsg.encode(6, MapleCharacter.makeMapleReadable(this.name) + " was autobanned for " + reason));
        } catch (RemoteException | NullPointerException ex) {
            Logger.log(LogType.ERROR, LogFile.REMOTE_EXCEPTION, ex);
        }
    ...
    // TODO: Check nhiệm vụ Multi Pets đã được làm xong
    public boolean isMultiPetActivated() {
        return false;
    }
    ...
    public void addAutoSellIgnore(String itemName) {
        ...
        if (!autoSellIgnore.stream().anyMatch(s -> s.contains(item))) {
            autoSellIgnore.add(item);
        }
    }
    ...
    public void addAutoSellInventoryIgnore(MapleInventoryType type) {
        if (!autoSellInventoryTypeIgnore.contains(type)) {
            autoSellInventoryTypeIgnore.add(type);
        }
    }
    ...
    public void gainRSSkillLevel(RSSkill skill, byte amount) {
                    ...
                    case Capacity:
                        ...
                        if (slots > 0) {
                            for (MapleInventoryType mit : MapleInventoryType.values()) {
                                if (!mit.equals(MapleInventoryType.CASH) && !mit.equals(MapleInventoryType.MESO) && !mit.equals(MapleInventoryType.EQUIPPED)) {
        ...
        if (level + amount <= skill.getMaxLevel()) {
            level += amount;
        } else {
        ...
        announce(EffectPacket.Local.ItemLevelUp());
        map.announce(EffectPacket.Remote.ItemLevelUp(id));
    }
    ...
    public void gainGP(int amount) {
        gp += amount;
        mgc.gainGP(amount);
        announce(WvsContext.OnMessage.getGPMessage(amount));
    }
    ...
    public MapleCharacter getPartner() {
        if (getMarriedTo() > 0) {
            return ChannelServer.getInstance().getCharacterById(getMarriedTo());
        } else {
            return null;
        }
    }
    ...
    public void removeAll(int id) {
        ...
        if (possessed > 0) {
            ...
            announce(EffectPacket.Local.Quest(id, (short) -possessed));
        }
        if (invType == MapleInventoryType.EQUIP) {
            if (getInventory(MapleInventoryType.EQUIPPED).countById(id) > 0) {
                ...
                announce(EffectPacket.Local.Quest(id, (short) -1));
            }
        }
    }
    ...
    public int getCapturedMob(int i) {
        return capturedMob.get(i);
    }

    public int getCapturedMobs() {
        return capturedMob.size();
    }

    public void addCapturedMob(int i) {
        capturedMob.add(i);
    }

    public int getPredictQuit() {
        return predictQuit;
    }

    public void setPredictQuit(int predictQuit) {
        this.predictQuit = predictQuit;
    }
}
