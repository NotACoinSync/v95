...
public class Equip extends Item {
    ...
    public void setStat(SecondaryStat stat, short amount) {
        switch (stat) {
            case PAD:
                setWatk(amount);
                break;
    ...
    public short getStat(SecondaryStat stat) {
        switch (stat) {
            case PAD:
                return getWatk();
            case MAD:
    ...
    public void gainLevel(MapleClient c, boolean timeless) {
        ...
        this.itemLevel++;
        c.announce(EffectPacket.Local.ItemLevelUp());
        c.getPlayer().getMap().announce(c.getPlayer(), EffectPacket.Remote.ItemLevelUp(c.getPlayer().getId()));
    ...
    public boolean handleGlass(Item glass) {
        // glass level check
        int maxLevel = 30;
        if (glass.getItemId() == 2460001) {
            maxLevel = 70;
        } else if (glass.getItemId() == 2460002) {
            maxLevel = 120;
        } else if (glass.getItemId() == 2460003) {
            maxLevel = 256;
        }
        ...
        if (option1 == 0 && option2 == 0 && option3 == 0) {// brand new pot item
            ...
            if (grade == 1 && Randomizer.nextInt(100) + 1 <= 10) {
                grade++;
            } else if (grade == 2 && Randomizer.nextInt(100) + 1 <= 1) {
                grade++;
            }
            int op1Grade = grade;
            ...
            if (grade == 2) {// epic
                ...
                if (Randomizer.nextInt(100) + 1 <= 25) {
                    op3Grade = 2;
                }
    ...
    private int generateLine(int grade) {
        ...
        while (line == 0) {
            ...
            if (data.reqLevel < pot.reqLevel) {
                continue;
            }
            if (!ItemConstants.isValidOptionType(pot, this.getItemId())) {
                continue;
            }
            // optionType check?
            // 1,2,3 is magnifying glass(To identify what grade after reveal), 4 prob a spacer for legendary
            ...
            if (grade == 1 || grade == 5) {// rare
                ...
                if (pot.id >= 901 && pot.id <= 905) {
                    line = pot.id;// face ones
                } else if (pot.id >= 20000 && pot.id <= 29999) {
                    line = pot.id;
                }
            } else if (grade == 3 || grade == 7) {// unique
                if (pot.id >= 901 && pot.id <= 905) {
                    line = pot.id;// faces ones
                } else if (pot.id >= 30000 && pot.id <= 39999) {
                    line = pot.id;
                }
            }
            if (line != 0) {
            ...
            if (++attempts >= ii.potentials.size()) {
                cycles++;
            }
            if (cycles >= 15) {
                pots = null;
