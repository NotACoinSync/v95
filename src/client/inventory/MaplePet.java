...
public class MaplePet extends Item {
    ...
    public void saveToDb() {
        try (PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("UPDATE pets SET name = ?, level = ?, closeness = ?, fullness = ?, summoned = ?, remainLife = ?, excluded = ? WHERE petid = ?")) {
            ...
            if (exc.toString().contains(",")) {
                exc.setLength(exc.length() - 1);
            }
    ...
    public void gainCloseness(MapleCharacter chr, int x) {
        ...
        if (closeness >= ExpTable.getClosenessNeededForLevel(getLevel())) {
            ...
            chr.getClient().announce(EffectPacket.Local.Pet(0, chr.getPetIndex(this)));
            chr.getMap().announce(EffectPacket.Remote.Pet(chr.getId(), 0, chr.getPetIndex(this)));
        }
    ...
    public void feed(MapleCharacter chr) {
        ...
        if (getFullness() < 100) {
            ...
            chr.getMap().announce(PetPacket.commandResponse(chr.getId(), chr.getPetIndex(this), 0, true));
            ...
            chr.getMap().announce(PetPacket.commandResponse(chr.getId(), chr.getPetIndex(this), 0, false));
            chr.getClient().announce(WvsContext.enableActions());
        }
    }
    ...
    public void addItemException(int x) {
        if (!aExceptionList.contains(x)) {
            aExceptionList.add(x);
        }
    }
