...
public class MapleClient {
    ...
    public void disconnectFully() {
        ...
        if (!isPlayerNull() && ChannelServer.getInstance() != null) {
            ...
            if (getChannelInstance() != null) {
                getChannelInstance().removePlayer(getPlayer());
            }
        }
        updateLoginState(MapleClient.LOGIN_NOTLOGGEDIN);
    ...
    public void sendCharList(int server) {
        this.announce(Login.SelectWorldResult(this, server));
    }
        ...
        try {
            ...
            if (game != null) {
                ...
                if (game.isOwner(player)) {
                    player.getMap().announce(UserCommon.MiniRoomBalloon.removeCharBox(player));
                    ...
                    if (!this.serverTransition) { // meaning not changing channels
                        ...
                        if (guild != null) {
                            ...
                            if (guild.getAllianceId() > 0) {
                                ChannelServer.getInstance().getWorldInterface().allianceMessage(guild.getAllianceId(), WvsContext.AllianceResult.allianceMemberOnline(player, false), player.getId(), -1);
                            }
                            announce(WvsContext.GuildResult.showGuildInfo(player));
                        }
                        if (party != null) {
    ...
    public boolean deleteCharacter(int world, int cid) {
        Connection con = DatabaseConnection.getConnection();
        try {
            if (LoginServer.getInstance().getCenterInterface().disconnectCharacter(world, cid)) {
                return false;
            }
        } catch (Exception re) {
        ...
        try {
            ...
            /*
             try (PreparedStatement ps = con.prepareStatement("DELETE FROM
             wishlists WHERE charid = ?")) {
             ps.setInt(1, cid);
             ps.executeUpdate();
             }
             try (PreparedStatement ps = con.prepareStatement("DELETE FROM
             ...
             String[] toDel = {"famelog", "inventoryitems", "keymap",
    ...
    public void sendPing() {
        ...
        announce(ClientSocket.AliveReq());
    ...
    public void changeChannel(int channel) {
        ...
        if (!player.isAlive() || FieldLimit.CHANGECHANNEL.check(player.getMap().getMapData().getFieldLimit())) {
            announce(WvsContext.enableActions());
            return;
        }
        ...
        if (player.getBuffedValue(SecondaryStat.PUPPET) != null) {
            player.cancelEffectFromSecondaryStat(SecondaryStat.PUPPET);
        }
        if (player.getBuffedValue(SecondaryStat.ComboCounter) != null) {
            player.cancelEffectFromSecondaryStat(SecondaryStat.ComboCounter);
        }
        player.getInventory(MapleInventoryType.EQUIPPED).checked(false); // test
        player.getMap().removePlayer(player);
        player.getClient().getChannelServer().removePlayer(player);
        if (sameChannelServer) {
            ChannelServer.getInstance().addPlayerToTempStorage(player);
        }
        player.getClient().updateLoginState(MapleClient.LOGIN_SERVER_TRANSITION);
        try {
            announce(ClientSocket.MigrateCommand(InetAddress.getByName(socket[0]), Integer.parseInt(socket[1])));
        } catch (IOException e) {
            Logger.log(LogType.ERROR, LogFile.EXCEPTION, e);
        }
    }
    ...
    public void addEliteDays(int days) {
        ...
        if (eliteStart == 0) {
            eliteStart = Calendar.getInstance().getTimeInMillis();
        }
            ...
            for (MapleInventoryType type : MapleInventoryType.values()) {
                ...
                for (Item item : inv.listById(itemID)) {
                    ...
                    if (expirationUpdate) {
                        ...
                        announce(WvsContext.InventoryOperation(true, Collections.singletonList(new ModifyInventory(0, item))));
                    }
                }
            }
            if (!found) {
    ...
    public int canChangeName() {
        if (eliteLength < (5 * 24 * 60 * 60 * 1000L)) {
            return 1;// 5 days of elite
        }
        if (lastNameChange == 0) {
            return 0;// if they never changed their name, allow em.
        }
        if ((Calendar.getInstance().getTimeInMillis() - lastNameChange) < 30 * 24 * 60 * 60 * 1000L) {
            return 2;// check if they changed it in the last 30 days
        }
        return 0;// we gud
    }
    ...
    public void parseMac(String rawMacs) {
        if (rawMacs == null || rawMacs.isEmpty()) {
            return;
        }
        for (String mac : rawMacs.trim().split(",")) {
            if (!mac.isEmpty()) {
                this.macs.add(mac.trim());
            }
        }
    }
