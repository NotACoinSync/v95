...
public class CharacterStat {
    ...
    public void addCritDamage(double critPercentage) {
        this.critDamage += critPercentage;
    }

    public void setFrom(MapleCharacter chr) {
        if (chr.getJob().getId() >= 500 && chr.getJob().getId() <= 522) {
            if (chr.getInventory(MapleInventoryType.EQUIPPED).getItem((short) EquipSlot.WEAPON.getSlots()[0]) != null) {
                accuracy += 10;
            }
        ...
        applyBuffStats(chr);
        //
        if (chr.getJob().isWarrior() || chr.getJob().isBeginner()) {
            accuracy += ((double) chr.getTotalDex() * 0.8D);
        } else if (chr.getJob().isThief() || chr.getJob().isArcher()) {
            accuracy += ((double) chr.getTotalDex() * 0.6D);
        } else if (chr.getJob().isPirate()) {
            accuracy += ((double) chr.getTotalDex() * 0.5D);
        }
        //
        if (chr.getJob().isWarrior() || chr.getJob().isThief() || chr.getJob().isArcher() || chr.getJob().isBeginner()) {
            accuracy += ((double) chr.getTotalLuk() * 0.5D);
        } else if (chr.getJob().isMagician()) {
            accuracy += ((double) chr.getTotalLuk() * 0.3D);
        } else if (chr.getJob().isPirate()) {
            accuracy += ((double) chr.getTotalLuk() * 0.2D);// I guess?
        }		//
        if (accuracyR > 0) {
            accuracy += accuracy * (accuracyR / 100);
        }
    ...
    private void applyBuffStats(MapleCharacter chr) {
        Integer acc = chr.getBuffedValue(SecondaryStat.ACC);
        if (acc != null) {
            accuracy += acc;
        }
        MapleStatEffect buff = chr.getStatForBuff(SecondaryStat.SharpEyes);
        if (buff != null) {
            critDamage += buff.getY() / 100D;
        }
    }
    ...
    /**
     * Apply potential stats from the provided Equip Does both basic and %
     * stats.
     */
