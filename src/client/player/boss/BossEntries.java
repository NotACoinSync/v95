...
public class BossEntries {
    ...
    /**
     * @return Index of the next available entry, -1 if no entry index is
     * available..
     */
    public byte getEntryIndex(BossEntryType type) {
        ...
        for (long end : entries) {
            if (end + (type.getReset() * 60 * 60 * 1000L) <= current) {
                return pos;
            }
            pos++;
        }
        if (entries.size() < type.getEntries()) {
            return pos;
        }
        return -1;
    }
    ...
    /**
     * Adds an entry. Index is grabbed using
     * {@link BossEntries#getEntryIndex(BossEntryType)}
     *
    ...
    public void addEntry(BossEntryType type, byte index) {
        ...
        if (entries.size() < type.getEntries()) {
            entries.add(index, System.currentTimeMillis());
        } else {
            entries.set(index, System.currentTimeMillis());
        }
            ...
            for (String bossEntry : bossEntryString.split(",")) {
                ...
                for (String entryTimeS : split[1].split("\\.")) {
                    if (entryTimeS != null && !entryTimeS.isEmpty()) {
                        entries.add(ObjectParser.toLong(entryTimeS));
                    }
                }
