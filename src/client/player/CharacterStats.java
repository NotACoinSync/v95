...
public class CharacterStats {
    ...
    public void recalcLocalStats(MapleCharacter chr) {
        ...
        resetLocalStats(chr);
        // expMod += 1000;
        Integer holySymbol = chr.getBuffedValue(SecondaryStat.HolySymbol);
        if (holySymbol != null) {
            bonusExpBuff += holySymbol.doubleValue();
            // expMod *= 1.0 + (holySymbol.doubleValue() / 100);
        }
        Integer mesoup = chr.getBuffedValue(SecondaryStat.MesoUp);
        if (mesoup != null) {
        ...
        if (chr.getBuffedValue(SecondaryStat.MesoUpByItem) != null) {
            mesoBuff += chr.getBuffEffect(SecondaryStat.MesoUpByItem).getProb();
        }
        if (chr.getBuffedValue(SecondaryStat.ItemUpByItem) != null) {
            dropBuff += chr.getBuffEffect(SecondaryStat.ItemUpByItem).getProb();
        }
        ...
        if (chr.getItemQuantity(4030002, false) > 0) {
            expMod += chr.getItemQuantity(4030002, false);
        }
        int rank = 0;
        double expRank = 20;
        ...
        Integer booster = chr.getBuffedValue(SecondaryStat.Booster);
        Integer speedInfusion = chr.getBuffedValue(SecondaryStat.PartyBooster);
