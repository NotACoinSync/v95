...
import net.server.channel.handlers.MigrateInHandler;
            ...
            try (PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT id FROM characters where name = ? AND deleted = 0 AND gm >= ?" + (admin ? "" : " AND accountid = ?"))) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ...
                        MaplePacketLittleEndianWriter oPacket = new MaplePacketLittleEndianWriter(4);
                        oPacket.writeInt(rs.getInt(1));
                        SeekableLittleEndianAccessor slea = new GenericSeekableLittleEndianAccessor(new ByteArrayByteStream(oPacket.getPacket()));
                        new MigrateInHandler().handlePacket(slea, c);
                    }
                }
            } catch (Exception ex) {
