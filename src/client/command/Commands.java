...
public class Commands {
    ...
    public static boolean executePlayerCommand(MapleClient c, String[] sub, char heading) {
        ...
        switch (sub[0]) {
            case "help":
            case "commands":
                player.yellowMessage("Mèo Commands:");
    ...
    public static boolean executeGMCommand(MapleClient c, String[] sub) {
        ...
        /*if(sub[0].equals("stun")){
			...
			victim.cancelBuffStats(SecondaryStat.Stun);
		}else */
        if (sub[0].equals("stunmap")) {
            ...
            for (MapleCharacter players : player.getMap().getCharacters()) {
                ...
                players.cancelBuffStats(SecondaryStat.Stun);
            }
            ...
            for (MapleCharacter chrs : player.getMap().getCharacters()) {
                if (!chrs.isGM()) {
                    ...
                    chrs.getClient().announce(UserLocal.SitResult(-1));
                    chrs.getMap().announce(chrs, UserRemote.setActivePortableChair(chrs.getId(), 0), false);
            ...
            for (MapleCharacter players : player.getMap().getCharacters()) {
                ...
                players.cancelBuffStats(SecondaryStat.Attract);
            }
            ...
            if (sub.length >= 2) {
                MapleCharacter victim = cserv.getChannelServer().getCharacterByName(sub[1]);
                victim.setHpMp(0);
                // Server.getInstance().announceGM(CWvsContext.BroadcastMsg.encode(5, player.getName() + " used !kill on " + victim.getName()));
            }
            ...
            c.getPlayer().getMap().announce(WvsContext.enableActions());
            ...
            if (sub[1].equals("map")) {
                player.getMap().announce(WvsContext.BroadcastMsg.encode(7, joinStringFrom(sub, 2), 9010000));
            } else {
                // Server.getInstance().announce(CWvsContext.BroadcastMsg.encode(7, joinStringFrom(sub, 1), 9010000));
            }
            ...
            if (sub.length > 2) {
                // Server.getInstance().announce(CWvsContext.BroadcastMsg.encode(6, "[Event] An event has started in Channel " + player.getClient().getChannel() + "! Use @join to participate!"));
            }
            ...
            if (sub.length > 2) {
                // Server.getInstance().announce(CWvsContext.BroadcastMsg.encode(6, "[Event] Event entry has closed. Good luck to all who are participating!"));
            }
		 ...
		 player.dropMessage("There are a total of " + total + " players online.");
            ...
            if (!find) {
                ...
                c.announce(ScriptMan.AskMenu(NPCConversationManager.NpcReplayedByNpc, 9010000, (byte) 0, sb.toString()));
            }
            ...
            player.getMap().announce(Clock.Created(time, 2));
            if (sub.length > 2) {
                TimerManager.getInstance().schedule("clock", new Runnable() {
                    ...
                    public void run() {
                        ...
                        player.getMap().announce(Clock.Destroy());
                    }
                }, time * 1000);
            }
    ...
    public static class ShutDownTask extends Thread {
        ...
        public void startShutDown(int timeNeeded) {
            ...
            if (timeNeeded >= 0) {
                ChannelServer.getInstance().broadcastPacket(WvsContext.BroadcastMsg.encode(0, "Shutting down... in " + timeNeeded + " minute" + (timeNeeded > 1 ? "s" : "") + "."));
            }
            if (timeNeeded > 0) {
                for (Channel channel : ChannelServer.getInstance().getChannels()) {
                    for (MapleCharacter chr : channel.getPlayerStorage().getAllCharacters()) {
                        chr.getMap().announce(Clock.Created(ServerConstants.secondsLeft, 2));
                    }
                }
            }
