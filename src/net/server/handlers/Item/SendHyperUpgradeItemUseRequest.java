...
public final class SendHyperUpgradeItemUseRequest extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor iPacket, MapleClient c) {
        ...
        iPacket.readInt();
        short scrollSlot = iPacket.readShort();
        short equipSlot = iPacket.readShort();
        ...
        Item equipp = null;
        if (equipSlot >= 0) {
            equipp = c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem(equipSlot);
        } else {
            equipp = c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).getItem(equipSlot);
        }
        if (equipp == null) {
        ...
        c.getPlayer().getMap().announce(UserCommon.showItemHyperUpgradeEffect(c.getPlayer().getId(), success, cursed, enchantSkill, 0));
