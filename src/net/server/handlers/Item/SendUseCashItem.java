...
import net.server.handlers.Player.SendAbilityUpRequest;
...
import tools.packets.CField.MessageBoxPool;
...
import tools.packets.UIVega;
import tools.packets.WvsContext;

public final class SendUseCashItem extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor iPacket, MapleClient c) {
        ...
        if (System.currentTimeMillis() - player.getLastUsedCashItem() < 1000) {
            return;
        }        
        ...
        if (itemType == 505) { // AP/SP reset
            if (itemId > 5050000) {
                int SPTo = iPacket.readInt();
                int SPFrom = iPacket.readInt();
                ...
                if (masterLevel > 0 && player.getMasterLevel(skillSPTo) <= masterLevel) {
                    maxLevel = masterLevel;
                }
                int maxLevelFrom = skillSPFrom.getMaxLevel();
                int masterLevelFrom = skillSPFrom.getMasterLevel();
                if (masterLevelFrom > 0 && player.getMasterLevel(skillSPFrom) <= masterLevelFrom) {
                    maxLevelFrom = masterLevelFrom;
                }
                ...
                int APTo = iPacket.readInt();
                int APFrom = iPacket.readInt();
                switch (APFrom) {
                    case 64: // str
                        if (player.getStr() < 5) {
                            return;
                ...
                SendAbilityUpRequest.addStat(c, APTo);
                c.announce(WvsContext.StatChanged(statupdate, true, c.getPlayer()));
            }
            remove(c, pos);
            ...
            switch (itemId) {
                case 5060000:
                    // Item tag.
                    int equipSlot = iPacket.readShort();
                    if (equipSlot == 0) {
                        return;
                    }
                ...
                case 5061003:
                    // Sealing lock
                    MapleInventoryType type = MapleInventoryType.getByType((byte) iPacket.readInt());
                    eq = c.getPlayer().getInventory(type).getItem((short) iPacket.readInt());
                ...
                case 5060002:
                    // Incubator TODO
                    byte inventory2 = (byte) iPacket.readInt();
                    short slot2 = (short) iPacket.readInt();
                    ...
                    if (getIncubatedItem(c, itemId)) {
                        ...
                        remove(c, pos);
                        c.announce(WvsContext.IncubatorResult(item2));
                    }
                    return;
                case 5062000:
                    ...
                    short eqpPos = iPacket.readShort();
                    iPacket.readShort();// int?
                    ...
                    if (eqp == null) {
                        ...
                        c.getPlayer().getMap().announce(UserCommon.showItemUnreleaseEffect(c.getPlayer().getId(), false));
                        return;
                    }
                    Equip e = (Equip) eqp;
                    ...
                    if (c.getPlayer().canHoldItem(new Item(2430112, (short) 1))) {
                        ...
                        c.getPlayer().getMap().announce(UserCommon.showItemUnreleaseEffect(c.getPlayer().getId(), true));
                        return;
                    } else {
                        c.getPlayer().getMap().announce(UserCommon.showItemUnreleaseEffect(c.getPlayer().getId(), false));
                        return;
            ...
            iPacket.readInt(); // time stamp
            if (eq != null) {
                player.forceUpdateItem(eq);
                remove(c, pos);
            }
        } else if (itemType == 507) {
            boolean bWhisperIcon;
            ...
            switch (itemId / 1000 % 10) {
                case 1: // Megaphone
                    if (player.getLevel() > 9) {
                        player.getClient().getChannelServer().broadcastPacket(WvsContext.BroadcastMsg.encode(MessageType.MEGAPHONE.getValue(), medal + player.getName() + " : " + iPacket.readMapleAsciiString()));
                    } else {
                ...
                case 2: // Super megaphone
                    try {
                        ChannelServer.getInstance().getWorldInterface().broadcastPacket(WvsContext.BroadcastMsg.encode(MessageType.SUPER_MEGAPHONE.getValue(), c.getChannel(), medal + player.getName() + " : " + iPacket.readMapleAsciiString(), (iPacket.readByte() != 0)));
                ...
                case 4:// Skull
                    try {
                        ChannelServer.getInstance().getWorldInterface().broadcastPacket(WvsContext.BroadcastMsg.encode(MessageType.SkullSpeaker.getValue(), c.getChannel(), medal + player.getName() + " : " + iPacket.readMapleAsciiString(), (iPacket.readByte() != 0)));
                    } catch (RemoteException | NullPointerException ex) {
                        Logger.log(LogType.ERROR, LogFile.REMOTE_EXCEPTION, ex);
                    }
                    break;
                case 5: // Maple TV
                    ...
                    if (tvType != 1) {
                        if (tvType >= 3) {
                            ...
                            ear = 1 == iPacket.readByte();
                        } else if (tvType != 2) {
                            iPacket.readByte();
                        }
                        if (tvType != 4) {
                            victim = c.getChannelServer().getPlayerStorage().getCharacterByName(iPacket.readMapleAsciiString());
                        }
                    }
                    ...
                    if (megassenger) {
                        try {
                            ChannelServer.getInstance().getWorldInterface().broadcastPacket(WvsContext.BroadcastMsg.encode(3, c.getChannel(), medal + player.getName() + " : " + builder.toString(), ear));
                        } catch (RemoteException | NullPointerException ex) {
                            Logger.log(LogType.ERROR, LogFile.REMOTE_EXCEPTION, ex);
                        }
                    }
                ...
                case 6: // item megaphone
                    String pDlg = medal + c.getPlayer().getName() + " : " + iPacket.readMapleAsciiString();
                    bWhisperIcon = iPacket.readByte() == 1;
                    Item item = null;
                    if (iPacket.readByte() == 1) { // item
                        item = c.getPlayer().getInventory(MapleInventoryType.getByType((byte) iPacket.readInt())).getItem((short) iPacket.readInt());
                        if (item == null) { // hack
                    ...
                    try {
                        ChannelServer.getInstance().getWorldInterface().broadcastPacket(WvsContext.BroadcastMsg.ItemMegaphone(item, pDlg, c.getChannel(), bWhisperIcon));
                    } catch (RemoteException | NullPointerException ex) {
                        Logger.log(LogType.ERROR, LogFile.REMOTE_EXCEPTION, ex);
                    }
                    break;
                case 7: // triple megaphone
                    int lines = iPacket.readByte();
                    ...
                    for (int i = 0; i < lines; i++) {
                        msg2[i] = medal + c.getPlayer().getName() + " : " + iPacket.readMapleAsciiString();
                    }
                    bWhisperIcon = iPacket.readByte() == 1;
                    try {
                        ChannelServer.getInstance().getWorldInterface().broadcastPacket(WvsContext.BroadcastMsg.MultiMegaphone(msg2, c.getChannel(), bWhisperIcon));
                    } catch (RemoteException | NullPointerException ex) {
                        Logger.log(LogType.ERROR, LogFile.REMOTE_EXCEPTION, ex);
                    }
                    break;
            ...
            String message = iPacket.readMapleAsciiString(); // message, sepearated by 0A for lines can be split by ' '
            ...
            if (position == null || !player.getMap().getMapObjectsInRange(player.getPosition(), 5000, Arrays.asList(MapleMapObjectType.KITE)).isEmpty() || message.length() > 200) {
                c.announce(MessageBoxPool.CreateFailed());
                c.announce(WvsContext.enableActions());
                return;
            }
            Kite kite = new Kite(player, itemId, message);
            player.getMap().addMapObject(kite);
            player.getMap().announce(MessageBoxPool.MessageBoxEnterField(kite));
            remove(c, pos);
        } else if (itemType == 509) {
            String sendTo = iPacket.readMapleAsciiString();
            String msg = iPacket.readMapleAsciiString();
            try {
            ...
            player.getMap().announce(Field.FieldEffect.musicChange("Jukebox/Congratulation"));
            remove(c, pos);
        } else if (itemType == 512) {
            /*
             if(player.getMap().startMapEffect(ii.getMsg(itemId).replaceFirst("%s",
             c.getPlayer().getName()).replaceFirst("%s",
             ...
             c.getPlayer().dropMessage(MessageType.ERROR, "A MapEffect is
             currently already running. Please try again later.");
             }
             */
            ...
            String newName = iPacket.readMapleAsciiString();
            pet.setName(newName);
            pet.saveToDb();
            player.forceUpdateItem(item);
            player.getMap().announce(player, PetPacket.changePetName(player, newName, 1), true);
            c.announce(WvsContext.enableActions());
            remove(c, pos);
        } else if (itemType == 504) { // teleport rock
            String error1 = "Either the player could not be found or you were trying to teleport to an illegal location.";
            boolean vip = iPacket.readByte() == 1;
            ...
            for (byte i = 0; i < 3; i++) {
                ...
                if (pet != null) {
                    if (pet.canConsume(itemId)) {
                        ...
                        // Item item = player.getInventory(MapleInventoryType.CASH).getItem(pet.getPosition());
                        // player.forceUpdateItem(item);
                        player.getMap().announce(c.getPlayer(), PetPacket.commandResponse(player.getId(), i, 1, true), true);
                        remove(c, pos);
                        c.announce(WvsContext.enableActions());
                        break;
                    }
            ...
            player.setChalkboard(iPacket.readMapleAsciiString());
            player.getMap().announce(UserCommon.ADBoard(player.getId(), player.getChalkboard(), false));
            player.getClient().announce(WvsContext.enableActions());
        } else if (itemType == 539) {
            List<String> lines = new LinkedList<>();
            for (int i = 0; i < 4; i++) {
                lines.add(iPacket.readMapleAsciiString());
            }
            try {
                ChannelServer.getInstance().getWorldInterface().broadcastPacket(WvsContext.SetAvatarMegaphone(c.getPlayer(), itemId, medal, lines, c.getChannel(), (iPacket.readByte() != 0)));
                TimerManager.getInstance().schedule("mega", new Runnable() {

                    @Override
                    public void run() {
                        try {
                            ChannelServer.getInstance().getWorldInterface().broadcastPacket(WvsContext.ClearAvatarMegaphone());
                        } catch (RemoteException | NullPointerException ex) {
                            Logger.log(LogType.ERROR, LogFile.REMOTE_EXCEPTION, ex);
                        }
                    }
            ...
            MapleInventoryType type = MapleInventoryType.getByType((byte) iPacket.readInt());
            short slot = (short) iPacket.readInt();
            ...
            remove(c, pos);
            c.announce(WvsContext.enableActions());
            c.announce(UIItemUpgrade.sendHammerData(equip.getVicious()));
            ...
            iPacket.readInt();
            int equipSlot = iPacket.readInt();
            iPacket.readInt();
            int scrollSlot = iPacket.readInt();
            ...
            c.announce(UIVega.Fail());
            c.announce(WvsContext.enableActions());
        } else if (itemType == 523) {
            int searchedItem = iPacket.readInt();
            byte sortByPrice = iPacket.readByte();
            iPacket.readInt();// time?
            List<PlayerShop> shops = new ArrayList<>();
            List<Integer> shopOIDS = new ArrayList<>();
            List<MaplePlayerShopItem> items = new ArrayList<>();
            /*
             for(Channel ch :
             Server.getInstance().getChannelsFromWorld(c.getPlayer().getWorld())){
            ...
            // }
            // }
            c.announce(WvsContext.ShopScannerResult(c, sortByPrice, searchedItem, shops, items));
            ...
            if (skilldata.skills.isEmpty()) {
                player.getClient().announce(WvsContext.SkillLearnItemResult(player.getId(), skill, maxlevel, false, success));
                return;
            }
            Skill skill2 = null;
            ...
            player.getClient().announce(WvsContext.SkillLearnItemResult(player.getId(), skill, maxlevel, canuse, success));
        } else {
