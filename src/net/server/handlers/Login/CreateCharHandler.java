...
public final class CreateCharHandler extends AbstractMaplePacketHandler {
    ...
    public static void loadCreateCharItems() {
        ...
        for (MapleData type : charInfo.getChildren()) {
            ...
            for (JobType jType : JobType.values()) {
                if (type.getName().startsWith(jType.wzPrefix)) {
                    jobType = jType;
                }
            }
            if (jobType != null) {
                ...
                for (MapleData type2 : type.getChildren()) {
                    for (JobType jType : JobType.values()) {
                        if (type2.getName().startsWith(jType.wzPrefix)) {
                            jobType = jType;
                        }
                    }
                    if (jobType != null) {
    ...
    public static class CharData {
        ...
        public void load(MapleData data) {
            if (data.getName().contains("Male")) {
                gender = 0;
            } else if (data.getName().contains("Female")) {
                gender = 1;
            }
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        switch (job) {
            ...
            default:
                ...
                c.announce(Login.DeleteCharacterResult(0, 9));
                return;
        }
        ...
        if (!charData.face.contains(face)) {
            illegal = true;
        }
        if (!charData.hair.contains(hair)) {
            illegal = true;
        }
        if (!charData.hairColor.contains(hairColor)) {
            illegal = true;
        }
        if (!charData.skin.contains(skincolor)) {
            illegal = true;
        }
        if (!charData.top.contains(top)) {
            illegal = true;
        }
        if (!charData.bottom.contains(bottom)) {
            illegal = true;
        }
        if (!charData.shoes.contains(shoes)) {
            illegal = true;
        }
        if (!charData.weapon.contains(weapon)) {
            illegal = true;
        }
        ...
        c.announce(Login.CreateNewCharacterResult(newchar));
        try {
            LoginServer.getInstance().getCenterInterface().broadcastGMPacket(WvsContext.SetWeekEventMessage("[NEW CHAR]: " + c.getAccountName() + " has created a new character with IGN " + name));
