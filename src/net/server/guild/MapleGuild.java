...
public class MapleGuild implements Externalizable {
    ...
    public void writeToDB(boolean bDisband) {
        try {
            ...
            if (!bDisband) {
                ...
                ps.execute();
                ps.close();
                this.broadcast(WvsContext.GuildResult.guildDisband(this.id));
            }
        } catch (SQLException se) {
            Logger.log(LogType.ERROR, LogFile.EXCEPTION, se);
        }
    ...
    public void broadcast(final byte[] packet, int dontSendTo) {
        ...
        for (MapleGuildCharacter mgc : members) {
            if (mgc.getId() != dontSendTo) {
                sendTo.add(mgc.getId());
            }
        }
        try {
    ...
    public final void setOnline(MapleGuildCharacter mgc, boolean online, int channel) {
        ...
        if (bBroadcast) {
            this.broadcast(WvsContext.GuildResult.guildMemberOnline(id, mgc.getId(), online), mgc.getId());
        }
    }

    public void guildChat(String sourceName, int sourceid, String message) {
        BigBrother.guild(this, sourceName, message);
        this.broadcast(Field.GroupMessage(sourceName, message, 2), sourceid);
    }
    ...
    public int addGuildMember(MapleGuildCharacter mgc) {
        ...
        this.broadcast(WvsContext.GuildResult.newGuildMember(mgc));
        return 1;
    }

    public void leaveGuild(MapleGuildCharacter mgc) {
        this.broadcast(WvsContext.GuildResult.memberLeft(mgc, false));
        synchronized (members) {
            members.remove(mgc);
        }
    }
        ...
        synchronized (members) {
            ...
            while (itr.hasNext()) {
                ...
                if (mgc.getId() == cid && initiator.getGuildRank() < mgc.getGuildRank()) {
                    this.broadcast(WvsContext.GuildResult.memberLeft(mgc, true));
                    itr.remove();
                    try {
                        if (mgc.isOnline()) {
    ...
    public void changeRank(int cid, int newRank) {
        for (MapleGuildCharacter mgc : members) {
            if (cid == mgc.getId()) {
                ...
                this.broadcast(WvsContext.GuildResult.changeRank(mgc));
                return;
            }
        }
    }
    ...
    public void setGuildNotice(String notice) {
        this.notice = notice;
        writeToDB(false);
        this.broadcast(WvsContext.GuildResult.guildNotice(this.id, notice));
    }
    ...
    public void memberLevelJobUpdate(MapleGuildCharacter mgc) {
        for (MapleGuildCharacter member : members) {
            if (mgc.getId() == member.getId()) {
                ...
                broadcast(WvsContext.GuildResult.guildMemberLevelJobUpdate(mgc));
                break;
            }
        }
    }
    ...
    public void changeRankTitle(String[] ranks) {
        ...
        this.broadcast(WvsContext.GuildResult.rankTitleChange(this.id, ranks));
    ...
    public String increaseCapacity() {
        ...
        if (getMeso() < cost) {
            return "Your guild doesn't have enough mesos.";
        }
        capacity += 5;
        this.removeMeso(cost);
        this.writeToDB(false);
        this.broadcast(WvsContext.GuildResult.guildCapacityChange(this.id, this.capacity));
        return "good";
    }
    ...
    public boolean gainGP(int chrid, int amount) {
        if (gp + amount > 0) {
            ...
            this.gp += amount;
            this.writeToDB(false);
            this.guildMessage(WvsContext.GuildResult.updateGP(this.id, this.gp));
            return true;
        }
        return false;
    }
    ...
    public static MapleGuildResponse sendInvite(MapleClient c, String targetName) {
        ...
        mc.getClient().announce(WvsContext.GuildResult.guildInvite(c.getPlayer().getGuildId(), c.getPlayer().getName(), c.getPlayer().getLevel(), c.getPlayer().getJob().getId()));
        return null;
    }
    ...
    public static void displayGuildRanks(MapleClient c, int npcid) {
        try {
            ...
            try (PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT `name`, `GP`, `logoBG`, `logoBGColor`, `logo`, `logoColor` FROM guilds WHERE NOT `guildid` = '1' ORDER BY `GP` DESC LIMIT 50")) {
                rs = ps.executeQuery();
                c.announce(WvsContext.GuildResult.showGuildRanks(npcid, rs));
            }
            rs.close();
        } catch (SQLException e) {
