...
public final class MTSHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (slea.available() > 0) {
            ...
            switch (op) {
                case 2:
                    {
                        ...
                        if (i != null && c.getPlayer().getMeso() >= 5000) {
                            ...
                            try {
                                ...
                                c.announce(CITC.NormalItemResult.GetUserPurchaseItem_Done(getTransfer(c.getPlayer().getId())));
                                c.announce(CITC.NormalItemResult.GetUserSaleItem_Done(getNotYetSold(c.getPlayer().getId())));
                                return;
                                }
                                }
                                }
                                ...
                                c.announce(ITC.NormalItemResult.RegisterSaleEntry_Done());
                                c.announce(getMTS(c.getPlayer(), 1, 0, 0));
                                c.announce(PacketHelper.enableCSUse());
                                c.announce(ITC.NormalItemResult.GetUserPurchaseItem_Done(getTransfer(c.getPlayer().getId())));
                                c.announce(ITC.NormalItemResult.GetUserSaleItem_Done(getNotYetSold(c.getPlayer().getId())));
                        ...
                        try (PreparedStatement ps = con.prepareStatement("SELECT * FROM mts_items WHERE seller = ? AND transfer = 1  AND id= ? ORDER BY id DESC")) {
                            ...
                            try (ResultSet rs = ps.executeQuery()) {
                                if (rs.next()) {
                                    ...
                                    c.announce(getCart(c.getPlayer(), c.getPlayer().getId()));
                                    c.announce(getMTS(c.getPlayer(), c.getPlayer().getCurrentTab(), c.getPlayer().getCurrentType(), c.getPlayer().getCurrentPage()));
                                    c.announce(ITC.NormalItemResult.MoveITCPurchaseItemLtoS_Done(i.getQuantity(), i.getPosition()));
                                    c.announce(ITC.NormalItemResult.GetUserPurchaseItem_Done(getTransfer(c.getPlayer().getId())));
                                }
                            }
                        } catch (SQLException e) {
                ...
                case 18:
                    {
                        ...
                        /**
                        * Sword: 1 quantity 01 - item type? 1 == equip F0 DD 13 00 -
                        * itemid 00 00 80 05 BB 46 E6 17 02 - ? 0F 00 00 00 00 00 00 00
                        * 00 00 00 00 00 00 11 00 00 00 00 00 00 00 00 00 00 00 00 00
                        * 00 00 00 00 05 00 41 72 6E 61 68 - Equip owner 00 00 00 01 00
                        * 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 40 E0 FD 3B
                        * 37 4F 01 FF FF FF FF 02 00 00 00 - slot 01 00 00 00 -
                        * quantity E8 03 00 00 - starting bid 39 30 00 00 - direct
                        * purchase 18 - sale duration 01 - fee type F4 01 00 00 - bid
                        * increment
                        */
                        /**
                        * Snail Shell: Slot 3, 150 units, direct purchase: 4444, bid
                        * increment: 10, starting bid: 1000, time(hours): 24 02- item
                        * type 13 09 3D 00 - item id 00 00 80 05 BB 46 E6 17 02 C8 00
                        * 05 00 41 72 6E 61 68 00 00 03 00 00 00 - slot 96 00 00 00 -
                        * quantity E8 03 00 00 - starting bid 5C 11 00 00 - direct
                        * purchase 18 - sale duration 01 - fee type 0A 00 00 00 - bid
                        * increment
                        */
                        if (true) {
                        ...
                        try {
                            ...
                            c.announce(ITC.NormalItemResult.encode(ITCResCode.RegAuction_Failed));
                        }       try {
                            c.announce(ITC.NormalItemResult.encode(ITCResCode.RegAuction_Done));
                ...
                case 19:
                    {
                        ...
                        if (c.getPlayer().getCashShop().getCash(GameConstants.MAIN_NX_TYPE) >= total) {
                            ...
                            try (PreparedStatement ps = con.prepareStatement("SELECT sell_ends from mts_items WHERE id = ?")) {
                                ...
                                try (ResultSet rs = ps.executeQuery()) {
                                    if (rs.next()) {
                                        ...
                                        c.announce(ITC.NormalItemResult.encode(ITCResCode.BidAuction_Failed));
                                        c.announce(getMTS(c.getPlayer(), c.getPlayer().getCurrentTab(), c.getPlayer().getCurrentType(), c.getPlayer().getCurrentPage()));
                                        c.announce(PacketHelper.enableCSUse());
                                        c.announce(ITC.NormalItemResult.GetUserPurchaseItem_Done(getTransfer(c.getPlayer().getId())));
                                        c.announce(ITC.NormalItemResult.GetUserSaleItem_Done(getNotYetSold(c.getPlayer().getId())));
                            ...
                            try {
                                c.announce(ITC.NormalItemResult.encode(ITCResCode.BidAuction_Done));
                                c.announce(getMTS(c.getPlayer(), c.getPlayer().getCurrentTab(), c.getPlayer().getCurrentType(), c.getPlayer().getCurrentPage()));
                                c.announce(PacketHelper.enableCSUse());
                                c.announce(ITC.NormalItemResult.GetUserPurchaseItem_Done(getTransfer(c.getPlayer().getId())));
                                c.announce(ITC.NormalItemResult.GetUserSaleItem_Done(getNotYetSold(c.getPlayer().getId())));
                ...
                case 20:
                    {
                        // buy && buy from cart, and buy from auction
                        int id = slea.readInt(); // id of the item
                        Connection con = DatabaseConnection.getConnection();
                        try {
                                        ...
                                        if (c.getPlayer().getCashShop().getCash(4) >= price) { // FIX
                                            try (PreparedStatement pse = con.prepareStatement("UPDATE mts_items SET seller = ?, transfer = 1 WHERE id = ? AND transfer = 0 AND seller != ?")) {
                                                ...
                                                if (pse.executeUpdate() == 0) {// check if we updated anything, if not it could of already been sold/removed
                                                    ...
                                                    c.announce(ITC.NormalItemResult.encode(ITCResCode.BuyItem_Failed));
                                                    c.announce(PacketHelper.enableCSUse());
                                                    c.announce(getMTS(c.getPlayer(), c.getPlayer().getCurrentTab(), c.getPlayer().getCurrentType(), c.getPlayer().getCurrentPage()));
                                                    c.announce(ITC.QueryCashResult(c.getPlayer()));
                                                    c.announce(ITC.NormalItemResult.GetUserPurchaseItem_Done(getTransfer(c.getPlayer().getId())));
                                            ...
                                            if(victim.getCashShop().isOpened()) victim.announce(CITC.QueryCashResult(victim));
                                            }else{
                                            ...
                                            c.announce(PacketHelper.enableCSUse());
                                            c.announce(getMTS(c.getPlayer(), c.getPlayer().getCurrentTab(), c.getPlayer().getCurrentType(), c.getPlayer().getCurrentPage()));
                                            c.announce(ITC.NormalItemResult.BuyItem_Done());
                                            c.announce(ITC.QueryCashResult(c.getPlayer()));
                                            c.announce(ITC.NormalItemResult.GetUserPurchaseItem_Done(getTransfer(c.getPlayer().getId())));
                                            c.announce(ITC.NormalItemResult.GetUserSaleItem_Done(getNotYetSold(c.getPlayer().getId())));
                                            c.announce(WvsContext.enableActions());
                                        } else {
                                            c.announce(ITC.NormalItemResult.BuyItem_Failed());
                                        }
                                    }
                                }
                            }
    ...
    public byte[] getCart(MapleCharacter chr, int cid) throws SQLException {
        ...
        return ITC.NormalItemResult.GetITCList_Done(items, 4, 0, 0, pages, chr.getCurrentSortType(), chr.getCurrentSortColumn());
    }
    ...
    public static byte[] getMTS(MapleCharacter chr, int tab, int type, int page) throws SQLException {
        ...
        return ITC.NormalItemResult.GetITCList_Done(items, tab, type, page, pages, chr.getCurrentSortType(), chr.getCurrentSortColumn()); // resniff
    }
    ...
    public static byte[] getAuctionItems(MapleCharacter chr, int seller, int page) throws SQLException {
        ...
        return ITC.NormalItemResult.GetITCList_Done(items, 4, 3, page, pages, chr.getCurrentSortType(), chr.getCurrentSortColumn()); // resniff
    }
    ...
    public static byte[] getHistory(MapleCharacter chr, int page) throws SQLException {
        ...
        return ITC.NormalItemResult.GetITCList_Done(items, 4, 2, page, pages, chr.getCurrentSortType(), chr.getCurrentSortColumn()); // resniff
    }
    ...
    public byte[] getMTSSearch(MapleCharacter chr, int tab, int type, int cOi, String search, int page) throws SQLException {
        ...
        return ITC.NormalItemResult.GetITCList_Done(items, tab, type, page, pages, chr.getCurrentSortType(), chr.getCurrentSortColumn());
    }
    ...
    public static MTSItemInfo createItem(ResultSet rs) throws SQLException {
        ...
        if (SQLUtil.hasColumn(rs, "status")) {
