        ...
        switch (mode) {
            ...
            case 0x12: // /h, information by vana (and tele mode f1) ... hide ofcourse
                ...
                if (c.getPlayer().uiToggle) {
                    c.announce(UserLocal.SetStandAloneMode(c.getPlayer().gmFlyMode));
                    c.announce(UserLocal.SetDirectionMode(c.getPlayer().gmFlyMode));// Make ui invis
                }
                break;
            ...
            case 0x1E: // Warn
                ...
                if (target != null) {
                    target.getClient().announce(WvsContext.BroadcastMsg.encode(1, message));
                    c.announce(Field.AdminResult.getGMEffect((byte) mode, (byte) 1));
                } else {
                    c.announce(Field.AdminResult.getGMEffect((byte) mode, (byte) 0));
                }
                break;
