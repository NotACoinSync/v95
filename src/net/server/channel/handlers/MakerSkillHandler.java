...
public final class MakerSkillHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        switch (type) {
            case 1: { // Making Items
                ...
                if (itemId == 4031980 || itemId == 4032334 || itemId == 4032312 || itemId == 2041058 || itemId == 2040727
                        || // Other items that fall in "0" category
                    ...
                    c.announce(UserLocal.makerResult(success, randGem, (short) quantity, gem.getCost(), gem.getReqItems(), false, 0, 0, null));
                } else {
                    ...
                    if (canCreate(c, item) && !c.getPlayer().getInventory(ii.getInventoryType(itemId)).isFull()) {
                        ...
                        c.getPlayer().gainRSSkillExp(RSSkill.Crafting, 1);
                        c.announce(WvsContext.InventoryOperation(true, Collections.singletonList(new ModifyInventory(0, toGive))));
                        showMakerSuccess(c);
                        success = true;
                    } else {
                        showMakerFailed(c);
                    }
                    c.announce(UserLocal.makerResult(success, itemId, item.getQuantity(), item.getCost(), item.getReqItems(), catalyst, item.getCatalyst(), amtINCStatGem, INCStatGems));
                }
                break;
            }
            case 3: { // Monster Crystals
                ...
                if (data.lv != 0) {
                    if (c.getPlayer().getItemQuantity(itemId, false) > 99) {
                        ...
                        c.getPlayer().gainRSSkillExp(RSSkill.Crafting, 1);
                        showMakerSuccess(c);
                        c.announce(UserLocal.makerResultCrystal(crystalToGive(data.lv), itemId));
                    } else {
                        showMakerFailed(c);
                    }
                }
            ...
            case 4: { // Disassembling
                ...
                if (!data.isDropRestricted()) {
                    ...
                    c.announce(UserLocal.makerResultDesynth(itemId, cost, itemsGained));
                } else {
                    showMakerFailed(c);
                }
                break;
    ...
    private void showMakerSuccess(MapleClient c) {
        c.announce(EffectPacket.Local.ItemMaker(false));
        c.getPlayer().getMap().announce(c.getPlayer(), EffectPacket.Remote.ItemMaker(c.getPlayer().getId(), false));
    }

    private void showMakerFailed(MapleClient c) {
        c.announce(EffectPacket.Local.ItemMaker(true));
        c.getPlayer().getMap().announce(c.getPlayer(), EffectPacket.Remote.ItemMaker(c.getPlayer().getId(), false));
    }
}
