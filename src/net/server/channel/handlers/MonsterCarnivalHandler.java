...
public final class MonsterCarnivalHandler extends AbstractMaplePacketHandler {
 ...
 /*MapleCharacter chr = c.getPlayer();
		...
		if(chr.getEventInstance() != null){
			if(carnivalParty.getTeam() != 0 && carnivalParty.getTeam() != 1){
				chr.getMap().announce(MaplePacketCreator.leaveCPQ(chr));
			...
			if(chr.getCP() > getPrice(chr, tab, number)){
				if(tab == 0){ // SPAWNING
					if(carnivalParty.canSummon()){ // what is max in GMS?
						...
						chr.getMap().announce(MaplePacketCreator.updateCP(chr.getCP(), carnivalParty.getTotalCP()));
						chr.getMap().announce(MaplePacketCreator.updatePartyCP(carnivalParty));
					...
					if (chr.getMap().getMobBuffs().isEmpty() || chr.getMap().getMobBuffs() == null || !chr.getMap().getMobBuffs().contains(MobSkillFactory.getMobSkill(getMobStatusSkillId(number), 1))) {
					    ...
					    chr.getMap().announce(MaplePacketCreator.CPQMessage((byte) 4));
					    return;
					}*/
 /*for(MapleMonster mob : c.getPlayer().getEventInstance().getMapInstance(c.getPlayer().getMapId()).getMonsters()){
		...
		chr.getMap().announce(MaplePacketCreator.CPQMessage((byte) 1));
		}
		}else{
		chr.announce(MaplePacketCreator.CPQMessage((byte) 5));
		}*/
        c.getPlayer().announce(WvsContext.enableActions());
    }
    ...
    public SecondaryStat getDiseaseByNum(int num) {
        num++;
        switch (num) {
            case 1:
                return SecondaryStat.Darkness;
            case 2:
                return SecondaryStat.Weakness;
            case 3:
                return SecondaryStat.Curse;
            case 4:
                System.out.println("For sake of testing, we will avoid this disease.");
            // return MapleDisease.Poison;
            case 5:
                return SecondaryStat.Slow;
            case 6:
                return SecondaryStat.Seal;
            case 7:
                return SecondaryStat.Stun;
        }
        return null;
    }
