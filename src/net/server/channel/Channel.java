...
public final class Channel {
        ...
        try {
            ...
            try {
                ...
                for (MapleMap map : getMaps().values()) {
                    ...
                    if (!points.isEmpty()) {
                        oPacket.writeInt(map.getId());
                        oPacket.writeInt(points.size());
                        for (SpawnPoint sp : points) {
                            spawns++;
                            oPacket.writeInt(sp.getMonsterId());
                            oPacket.writePos(sp.getPosition());
                            oPacket.writeLong(sp.getNextPossibleSpawn());
                        }
                    }
                }
                if (spawns > 0) {
    ...
    public void addPlayer(MapleCharacter chr) {
        ...
        try {
            chr.announce(WvsContext.BroadcastMsg.encode(ChannelServer.getInstance().getWorldInterface().getServerMessage()));
    ...
    public void setServerMessage(String message) {
        broadcastPacket(WvsContext.BroadcastMsg.encode(message));
    }
    ...
    public void addExpEventTime(long time) {
        ...
        if (extended) {
            Server.getInstance().broadcastMessage(WvsContext.BroadcastMsg.encode(MessageType.NOTICE.getValue(), "The Maple Tree 1.5x exp boost has been extended for an additional 30 minutes and will end in " + end + "."));
        } else {
            Server.getInstance().broadcastMessage(WvsContext.BroadcastMsg.encode(MessageType.NOTICE.getValue(), "As the Maple Tree falls it has granted everyone a 1.5x exp boost for 30 minutes."));
        }
    }
