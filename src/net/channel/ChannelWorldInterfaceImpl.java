...
public class ChannelWorldInterfaceImpl extends UnicastRemoteObject implements ChannelWorldInterface {
    ...
    public CharacterLocation find(int chrid) throws RemoteException {
        ...
        if (chr != null) {
            return new CharacterLocation(chr);
        }
        return null;
    }
    ...
    public String getGuildInviteStatus(String charname) throws RemoteException {
        ...
        if (chr != null) {
            if (chr.getGuildId() == 0) {
                ...
                return "The player is not the leader of his/her guild.";
            } else {
                return "Good";
            }
        }
    ...
    public void setGuildAndRank(int cid, int guildid, int rank) throws RemoteException {
        ...
        if (bDifferentGuild) {
            if (guildid == 0) {
                mc.getMap().announce(mc, UserRemote.guildNameChanged(mc.getId(), ""));
            } else {
                MapleGuild guild = ChannelServer.getInstance().getWorldInterface().getGuildIfExists(guildid);
                mc.getMap().announce(mc, UserRemote.guildNameChanged(mc.getId(), guild != null ? guild.getName() : ""));
                if (guild != null) {
                    mc.getMap().announce(mc, UserRemote.guildMarkChanged(mc.getId(), guild));
                }
            }
        }
    }
            ...
            for (MaplePartyCharacter partychar : party.getMembers()) {
                if (partychar.getChannel() == ch.getId()) {
                    ...
                    if (chr != null) {
                        ...
                        chr.getClient().announce(WvsContext.PartyResult.updateParty(chr.getClient().getChannel(), party, operation, target));
                    }
                }
            }
            switch (operation) {
        ...
        for (int buddy : buddies) {
            ...
            if (chr != null) {
                ...
                if (ble != null && ble.isVisible()) {
                    ...
                    chr.getClient().announce(WvsContext.FriendResult.updateBuddyChannel(ble));
                }
            }
        }
    }
        ...
        for (int characterId : recipientCharacterIds) {
            ...
            if (chr != null) {
                if (chr.getBuddylist().containsVisible(sourceid)) {
                    chr.getClient().announce(Field.GroupMessage(source, chattext, 0));
                }
            }
        }
    }
            ...
            for (MapleMessengerCharacter messengerchar : messenger.getMembers()) {
                if (messengerchar.getChannel() == channel.getId() && !(messengerchar.getName().equals(namefrom))) {
                    ...
                    if (chr != null) {
                        ...
                        if (mcl != null) {
                            chr.getClient().announce(UIMessenger.Enter(mcl, namefrom, position, fromchannel));
                            ChannelServer.getInstance().getWorldInterface().broadcastPacket(Arrays.asList(mcl.getId()), UIMessenger.Enter(chr, messengerchar.getPosition()));
                        }
                    }
                }
            }
            ...
            for (MapleMessengerCharacter messengerchar : messenger.getMembers()) {
                if (messengerchar.getChannel() == channel.getId() && !(messengerchar.getName().equals(namefrom))) {
                    ...
                    if (chr != null) {
                        ...
                        chr.getClient().announce(UIMessenger.Migrated(messenger));
                    }
                }
            }
        }
    ...
    public MapleCharacterLook getCharacterLooks(String character) throws RemoteException {
        ...
        if (mc != null) {
            return new MapleCharacterLook(mc);
        }
        return null;
    }
