...
public class WorldServer {
    ...
    public void shutdown() {
        ...
        try {
            if (centerRegistry != null) {
                centerRegistry.removeWorldServer(worldServerID);
            }
    ...
    public void setAllianceRank(int chrid, int rank) throws RemoteException {
        for (ChannelWorldInterface cwi : WorldRegistryImpl.getInstance().getChannelServers().values()) {
            if (cwi.setAllianceRank(chrid, rank)) {
                return;
            }
    ...
    public int[] getTopGuilds() {
        ...
        for (MapleGuild guild : guilds.values()) {
            if (guild.getName() != null && !guild.getName().equalsIgnoreCase("Vertisy") && guild.getName().length() > 0) {
                allGuilds.put(guild.getId(), guild.getGP());
            }
        }
    ...
    public void changeEmblem(int gid, List<Integer> affectedPlayers, MapleGuildSummary mgs) throws RemoteException {
        ...
        try {
            for (ChannelWorldInterface cwi : WorldRegistryImpl.getInstance().getChannelServers().values()) {
                cwi.broadcastPacket(affectedPlayers, WvsContext.GuildResult.guildEmblemChange(gid, mgs.getLogoBG(), mgs.getLogoBGColor(), mgs.getLogo(), mgs.getLogoColor()));
            }
        } catch (RemoteException | NullPointerException ex) {
    ...
    public void partyChat(MapleParty party, String chattext, String source) throws RemoteException {
        ...
        broadcastPacketToPlayers(targets, Field.GroupMessage(source, chattext, 1));
    }
            ...
            switch (operation) {
                case ADDED:
                    if (buddylist.contains(cidFrom)) {
                        ...
                        for (ChannelWorldInterface cwi : WorldRegistryImpl.getInstance().getChannelServers().values()) {
                            if (cwi.addToBuddylist(cid, entry)) {
                                continue;
                            }
                        }
                        broadcastPacket(target, WvsContext.FriendResult.updateBuddyChannel(entry));
                    }
                    break;
                case DELETED:
                    if (buddylist.contains(cidFrom)) {
    ...
    public BuddyAddResult requestBuddyAdd(int chrid, int channelFrom, int cidFrom, String nameFrom) throws RemoteException {
        ...
        if (buddylist != null) {
            ...
            if (!buddylist.contains(cidFrom)) {
                for (ChannelWorldInterface cwi : WorldRegistryImpl.getInstance().getChannelServers().values()) {
                    if (cwi.addBuddyRequest(chrid, cidFrom, nameFrom, channelFrom)) {
                        break;
                    }
                }
    ...
    public void messengerInvite(String sender, int messengerid, String target, int fromchannel) throws RemoteException {
        if (!isInAMessenger(target)) {
            broadcastPacketToPlayers(Arrays.asList(target), UIMessenger.Invite(sender, messengerid, fromchannel));
            broadcastPacketToPlayers(Arrays.asList(sender), UIMessenger.InviteResult(target, true));
        } else {
            broadcastPacketToPlayers(Arrays.asList(sender), UIMessenger.Chat(sender + " : " + target + " is already using Maple Messenger"));
        }
    }
    ...
    public void removeMessengerPlayer(MapleMessenger messenger, int position) throws RemoteException {
        ...
        if (!sendTo.isEmpty()) {
            this.broadcastPacket(sendTo, UIMessenger.Leave(position));
        }
    }
    ...
    public void messengerChat(int messengerid, String chattext, String namefrom) throws RemoteException {
        ...
        if (!target.isEmpty()) {
            broadcastPacket(target, UIMessenger.Chat(chattext));
        }
        BigBrother.messenger(from, to1, to2, chattext);
    }

    public void declineChat(String target, String namefrom) throws RemoteException {
        broadcastPacketToPlayers(Arrays.asList(target), UIMessenger.Blocked(namefrom, true));
    }
