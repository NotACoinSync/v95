...
public class QuestScriptManager extends AbstractScriptManager {
    ...
    public void start(MapleClient c, short questid, int npc) {
        ...
        try {
            if (c.getPlayer().getScriptDebug()) {
                c.getPlayer().dropMessage(MessageType.MAPLETIP, "Running script: " + quest.startQuestData.script + " Quest ID: " + questid + ", NPC: " + npc);
            }
    ...
    public void end(MapleClient c, short questid, int npc) {
        ...
        try {
            if (c.getPlayer().getScriptDebug()) {
                c.getPlayer().dropMessage(MessageType.MAPLETIP, "Running script: " + quest.completeQuestData.script + " Quest ID: " + questid + ", NPC: " + npc);
            }
            ...
            if (c.canClickNPC()) {
                ...
                ScriptEngine se = getInvocable("quest/" + questid + ".js", c);
                if (se == null) {
                    se = getInvocable("quest/" + quest.completeQuestData.script + ".js", c);
                }
                if (se == null) {
                    se = getInvocable("quest/world" + c.getWorld() + "/" + questid + ".js", c);
                }
                if (se == null) {
                    se = getInvocable("quest/world" + c.getWorld() + "/" + quest.completeQuestData.script + ".js", c);
                }
                if (se == null) {
                    qm.dispose();
                    return;
