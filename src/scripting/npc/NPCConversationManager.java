...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    public void sendSay(int nSpeakerTemplateID, String text, byte speaker, boolean bPrev, boolean bNext) {
        ...
        if (talkData.validSelections.isEmpty()) {
            getClient().announce(ScriptMan.Say(NpcReplayedByNpc, nSpeakerTemplateID, speaker, text, bPrev, bNext));
        } else {
            talkData.messageType = ScriptMessageType.AskMenu;
            getClient().announce(ScriptMan.AskMenu(NpcReplayedByNpc, nSpeakerTemplateID, speaker, text));
        }
    }
    ...
    public void sendSayUser(int nSpeakerTemplateID, String text, boolean bPrev, boolean bNext) {
        ...
        talkData.next = bNext;
        if (talkData.validSelections.isEmpty()) {
            getClient().announce(ScriptMan.Say(NpcReplayedByNpc, nSpeakerTemplateID, NpcReplacedByUser, text, bPrev, bNext));
        } else {
            talkData.messageType = ScriptMessageType.AskMenu;
            getClient().announce(ScriptMan.AskMenu(NpcReplayedByNpc, nSpeakerTemplateID, NpcReplacedByUser, text));
        }
    }
    ...
    public void sendSay(int nSpeakerTypeID, int nSpeakerTemplateID, String text, byte speaker, boolean bPrev, boolean bNext) {
        ...
        talkData.prev = bPrev;
        talkData.next = bNext;
        getClient().announce(ScriptMan.Say(nSpeakerTypeID, nSpeakerTemplateID, speaker, text, bPrev, bNext));
    }
    ...
    public void sendOk(int nSpeakerTemplateID, String text, byte speaker) {
        ...
        talkData.messageType = ScriptMessageType.Say;
        talkData.parseText(text);
        getClient().announce(ScriptMan.Say(NpcReplayedByNpc, nSpeakerTemplateID, speaker, text, false, false));
    }
    ...
    public void sendYesNo(int nSpeakerTemplateID, String text, byte speaker) {
        ...
        talkData.next = true;
        talkData.prev = true;
        getClient().announce(ScriptMan.AskYesNo(NpcReplayedByNpc, nSpeakerTemplateID, speaker, text));
    }
    ...
    public void sendAcceptDecline(int nSpeakerTemplateID, String text, byte speaker) {
        ...
        talkData.next = true;
        talkData.prev = true;
        getClient().announce(ScriptMan.AskAccept(NpcReplayedByNpc, nSpeakerTemplateID, speaker, text));
    }
    ...
    public void sendStyle(int nSpeakerTemplateID, String text, int aCanadite[]) {
        ...
        talkData.parseText(text);
        getClient().announce(ScriptMan.AskMembershopAvatar(NpcReplayedByNpc, nSpeakerTemplateID, text, aCanadite));
    }
    ...
    public void sendGetNumber(int nSpeakerTemplateID, String text, int def, int min, int max) {
        ...
        talkData.parseText(text);
        talkData.next = true;
        getClient().announce(ScriptMan.AskNumber(NpcReplayedByNpc, nSpeakerTemplateID, (byte) 0, text, def, min, max));
    }
    ...
    public void sendGetText(int nSpeakerTemplateID, String text, String sMsgDefault, int nLenMin, int nLenMax) {
        ...
        talkData.parseText(text);
        talkData.next = true;
        getClient().announce(ScriptMan.AskText(NpcReplayedByNpc, nSpeakerTemplateID, (byte) 0, text, sMsgDefault, nLenMin, nLenMax));
    }
    ...
    public void sendPets(String text, List<Item> pets) {
        ...
        talkData.parseText(text);
        talkData.next = true;
        getClient().announce(ScriptMan.AskPet(NpcReplayedByNpc, npc, text, pets));
    }
    ...
    public void sendPetsAll(String text, List<Item> pets) {
        ...
        talkData.parseText(text);
        talkData.next = true;
        getClient().announce(ScriptMan.AskPetAll(NpcReplayedByNpc, npc, text, pets, false));
    }
    ...
    public void sendDimensionalMirror(String text) {
        ...
        talkData.parseText(text);
        talkData.next = true;
        getClient().announce(ScriptMan.AskSlideMenu(0x0, npc, false, 0, text));
    ...
    public void sendSpeedQuiz(byte result, byte isMob, int objectID, int questions, int points, int timeLimit) {
        ...
        talkData.next = true;
        getClient().announce(ScriptMan.getSpeedQuiz(npc, result, isMob, objectID, questions, points, timeLimit));
    }
    ...
    public void showEffect(String effect) {
        getPlayer().getMap().announce(Field.FieldEffect.environmentChange(effect, 3));
    }
    ...
    public void doGachapon() {
        ...
        String strFieldname = c.getChannelServer().getMap(maps[(getNpc() != 9100117 && getNpc() != 9100109) ? (getNpc() - 9100100) : getNpc() == 9100109 ? 8 : 9]).getMapData().getMapName();
        BigBrother.logGacha(getPlayer(), item.getId(), strFieldname);
        if (item.getTier() > 1) { // Uncommon and Rare
            try {
                ChannelServer.getInstance().getWorldInterface().broadcastPacket(WvsContext.BroadcastMsg.Gachapon(getPlayer(), itemGained, strFieldname));
    ...
    public void disbandAlliance(MapleClient c, int allianceId) {
        ...
        try {
            ...
            ps.close();
            try {
                ChannelServer.getInstance().getWorldInterface().allianceMessage(c.getPlayer().getGuild().getAllianceId(), WvsContext.AllianceResult.disbandAlliance(allianceId), -1, -1);
    ...
    public MapleAlliance createAlliance(MapleCharacter chr1, MapleCharacter chr2, String name) {
        ...
        try {
            ...
            try {
                ...
                ChannelServer.getInstance().getWorldInterface().allianceMessage(id, WvsContext.AllianceResult.makeNewAlliance(alliance, chr1.getClient()), -1, -1);
            } catch (RemoteException | NullPointerException ex) {
                Logger.log(LogType.ERROR, LogFile.REMOTE_EXCEPTION, ex);
            }
        } catch (Exception e) {
    ...
    public void showFredrick() {
        c.announce(StoreBankDlg.Get(getPlayer()));
    }
    ...
    public final void closeUI(int map) {
        c.getChannelServer().getMap(map).announce(UserLocal.SetStandAloneMode(false));
    }
    ...
    public SlayerTask createSlayerTask() {
        ...
        while (task == null) {
            ...
            if (((getLevel() > MobConstants.HIGH_LEVEL_MOB && monsterLevel > MobConstants.HIGH_LEVEL_MOB) || (level <= levelRange && level >= -levelRange)) && rand.nextBoolean()) {
                if (MapleLifeFactory.getMonster(monsterID).getStats().getLink() == 0) {
                    task = new SlayerTask(monsterID, monsterLevel, data.right);
                }
            }
            if (++calls >= maxCalls) {
                levelRange += 2;
    ...
    public String getSlayerMobImg(int mob) {
        ...
        if (monster.getStats().getDefaultMoveType() != null) {
            return "#fMob/" + mobStr + ".img/" + monster.getStats().getDefaultMoveType() + "/0#";
        } else {
            return "";
        }
    }
    ...
    public boolean isInvitedToCurrentWedding(boolean cathedral) {
        ...
        if (wedding != null) {
            return wedding.getInvited().contains(getPlayer().getId());
        }
        return false;
    }
    ...
    public void removeCurrentWedding(boolean cathedral) {
        if (cathedral) {
            getClient().getChannelServer().setCurrentCathedralMarriageID(-1);
        } else {
            getClient().getChannelServer().setCurrentChapelMarriageID(-1);
        }
    }
    ...
    public void startWedding(EventInstanceManager eim) {// Sets are current wedding in channel, sets wedding status to 1,
        ...
        wedding.setEIM(eim);
        wedding.setState(1);
        if (wedding.isCathedral()) {
            getClient().getChannelServer().setCurrentCathedralMarriageID(getPlayer().getMarriageID());
        } else {
            getClient().getChannelServer().setCurrentChapelMarriageID(getPlayer().getMarriageID());
        }
    ...
    public int isWeddingCouple() {
        ...
        if (wedding != null) {
            if (wedding.getPlayer1() == getPlayer().getId() || wedding.getPlayer2() == getPlayer().getId()) {
                return 1;
            } else {
                return 0;
            }
