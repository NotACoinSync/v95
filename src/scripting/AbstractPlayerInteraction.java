...
public class AbstractPlayerInteraction {
        ...
        for (MaplePartyCharacter mpc : getPlayer().getParty().getMembers()) {
            if (mpc.isOnline()) {
                ...
                if (mc != null) {// incase somehow.
                    if (inMap) {
                        if (mc.getMapId() != getPlayer().getMapId()) {
                            break;
                        }
                    }
                    chars.add(mc);
    ...
    public final boolean canHoldMerchantItems(List<Pair<Item, MapleInventoryType>> items) {
        ...
        if (itemList.size() > 0) {
            return getPlayer().canHoldItems(itemList);
        } else {
            return true;
        }
    }
    ...
    public void spawnNpc(int npcId, Point pos) {
        ...
        if (npc != null) {
            ...
            c.getPlayer().getMap().announce(NpcPool.NpcEnterField(npc));
        }
    }
    ...
    public Item gainItem(MapleCharacter mc, int id, short quantity, boolean randomStats, boolean showMessage, long expires, boolean checkSpace, String gainLog) {
        ...
        if (showMessage) {
            mc.getClient().announce(EffectPacket.Local.Quest(id, quantity));
        }
    ...
    public void changeMusic(String songName) {
        getPlayer().getMap().announce(Field.FieldEffect.musicChange(songName));
    }

    public void playerMessage(int type, String message) {
        c.announce(WvsContext.BroadcastMsg.encode(type, message));
    }
    ...
    public void mapMessage(int type, String message) {
        getPlayer().getMap().announce(WvsContext.BroadcastMsg.encode(type, message));
    }

    public void mapEffect(String path) {
        c.announce(Field.FieldEffect.mapEffect(path));
    }

    public void mapSound(String path) {
        c.announce(Field.FieldEffect.mapSound(path));
    }
    ...
    public void showIntro(String path) {
        c.announce(EffectPacket.Local.ShowEffectByPath(path));
    }

    public void showInfo(String path) {
        c.announce(EffectPacket.Local.AvatarOriented(path));
        c.announce(WvsContext.enableActions());
    }

    public void guildMessage(int type, String message) {
        try {
            ChannelServer.getInstance().getWorldInterface().guildMessage(getPlayer().getGuildId(), WvsContext.BroadcastMsg.encode(type, message));
    ...
    public void givePartyItems(int id, short quantity, List<MapleCharacter> party) {
        for (MapleCharacter chr : party) {
            ...
            cl.announce(EffectPacket.Local.Quest(id, quantity));
        }
    }
        ...
        for (MaplePartyCharacter member : party.getMembers()) {
            ...
            if (player != null) {
                ...
                if (PQ != null && PQ.length() > 0) {
                    expBase = PartyQuest.getExp(PQ, player.getLevel());
                }
    ...
    public void removeFromParty(int id, List<MapleCharacter> party) {
        for (MapleCharacter chr : party) {
            ...
            if (possessed > 0) {
                MapleInventoryManipulator.removeById(c, ItemInformationProvider.getInstance().getInventoryType(id), id, possessed, true, true);
                chr.announce(EffectPacket.Local.Quest(id, (short) -possessed));
            }
        }
    }
    ...
    public int removeAll(int id, MapleClient c) {
        MapleInventoryType invType = ItemInformationProvider.getInstance().getInventoryType(id);
        int possessed = c.getPlayer().getInventory(invType).countById(id);
        if (possessed > 0) {
            MapleInventoryManipulator.removeById(c, ItemInformationProvider.getInstance().getInventoryType(id), id, possessed, true, true);
            c.announce(EffectPacket.Local.Quest(id, (short) -possessed));
        }
        if (invType == MapleInventoryType.EQUIP) {
            if (c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).countById(id) > 0) {
                MapleInventoryManipulator.removeById(c, MapleInventoryType.EQUIPPED, id, 1, true, true);
                c.announce(EffectPacket.Local.Quest(id, (short) -1));
                possessed++;
            }
        }
        return possessed;
    ...
    public void showInstruction(String msg, int width, int height) {
        c.announce(UserLocal.BalloonMsg(msg, width, height));
        c.announce(WvsContext.enableActions());
    }

    public void disableMinimap() {
        c.announce(Field.AdminResult.disableMinimap());
    }
    ...
    public boolean isAllReactorState(final int reactorId, final int state) { // Use for Thief Room CWKPQ
        ...
        for (MapleReactor reactor : getPlayer().getEventInstance().getMapInstance(getPlayer().getMapId()).getAllReactor()) {
            if (reactor.getId() == reactorId) {
                if (reactor.getCurrStateAsByte() != state) {
                    ret = false;
                }
            }
        }
    ...
    public void resetMap(int mapid) {
        ...
        for (MapleMapObject i : getMap(mapid).getMapObjectsInRange(c.getPlayer().getPosition(), Double.POSITIVE_INFINITY, Arrays.asList(MapleMapObjectType.ITEM))) {
            ...
            getMap(mapid).announce(DropPool.LeaveField(i.getObjectId(), 0, c.getPlayer().getId()));
        }
    }

    public void sendClock(MapleClient d, int time) {
        d.announce(Clock.Created((int) (time - System.currentTimeMillis()) / 1000, 2));
    }

    public void useItem(int id) {
        ItemInformationProvider.getInstance().getItemData(id).itemEffect.applyTo(c.getPlayer());
        c.announce(WvsContext.OnMessage.getItemMessage(id));// Useful shet :3
    }
    ...
    public void gainAndEquip(int itemid, short slot) {
        ...
        c.announce(WvsContext.InventoryOperation(false, Collections.singletonList(new ModifyInventory(0, newItem))));
    }
    ...
    public void spawnGuide() {
        c.announce(UserLocal.HireTutor(true));
    }

    public void removeGuide() {
        c.announce(UserLocal.HireTutor(false));
    }

    public void displayGuide(int num) {
        c.announce(EffectPacket.Local.ShowEffectByPath("UI/tutorial.img/" + num));
    }

    public void goDojoUp() {
        c.announce(UserLocal.Teleport((byte) 6, 0));
    }
    ...
    public void showEffect(String effect) {
        c.announce(Field.FieldEffect.showEffect(effect));
    }

    public void dojoEnergy() {
        c.announce(WvsContext.SessionValue("energy", getPlayer().getDojoEnergy()));
    }

    public void talkGuide(String message) {
        c.announce(UserLocal.talkGuides(message));
    }

    public void guideHint(int nIdx) {
        c.announce(UserLocal.TutorMsg(nIdx, 7000, "", true));
    }

    public void guideHint(int nIdx, String message) {
        c.announce(UserLocal.TutorMsg(nIdx, 7000, message, false));
    }

    public void updateAreaInfo(Short area, String info) {
        c.getPlayer().updateAreaInfo(area, info);
        c.announce(WvsContext.enableActions());// idk, nexon does the same :P
    }
    ...
    public void earnTitle(String msg) {
        c.announce(WvsContext.ScriptProgressMessage(msg));
    }

    public void showInfoText(String msg) {
        c.announce(WvsContext.OnMessage.showInfoText(msg));
    }

    public void openUI(byte ui) {
        c.announce(UserLocal.openUI(ui));
    }

    public void lockUI() {
        c.announce(UserLocal.SetStandAloneMode(true));
        c.announce(UserLocal.SetDirectionMode(true));
    }
    ...
    public void playSound(String sound) {
        getPlayer().getMap().announce(Field.FieldEffect.environmentChange(sound, 4));
    }

    public void environmentChange(String env, int mode) {
        getPlayer().getMap().announce(Field.FieldEffect.environmentChange(env, mode));
    }
        ...
        if (getPlayer().isInParty()) {
            for (MaplePartyCharacter mpc : getParty().getMembers()) {
                if (mpc.isOnline()) {
                    MapleCharacter chr = mpc.getPlayerInChannel();
                    if (chr != null) {
                        chr.getCashShop().gainCash(4, amount);
                    }
                }
            }
        }
    ...
    public void sendImage(String sPath) {
        getClient().announce(ScriptMan.SayImage(NPCConversationManager.NpcReplacedByUser, 0, (byte) 0, sPath));
    }
