    ...
    public void act(MapleClient c, MapleReactor reactor) {
        try {
            if (c.getPlayer().getScriptDebug()) {
                c.getPlayer().dropMessage(MessageType.MAPLETIP, "Reactor ID: " + reactor.getId() + " Action: " + reactor.getAction() + " Link: " + reactor.getLink());
            }
            ReactorActionManager rm = new ReactorActionManager(c, reactor);
            ScriptEngine se = null;
            if (reactor.getAction() != null) {
                se = getInvocable("reactor/" + reactor.getAction() + ".js", c);// can it be null?
            }
            if (se == null) {
                se = getInvocable("reactor/" + reactor.getId() + ".js", c);
            }
            if (se == null) {
                if (c.getPlayer().getScriptDebug()) {
                    c.getPlayer().dropMessage(MessageType.MAPLETIP, "Uncoded act Reactor ID: " + reactor.getId() + " Action: " + reactor.getAction() + " Link: " + reactor.getLink());
                }
