...
import tools.packets.CField.Wedding;
...
public class EventInstanceManager {
    ...
    public void sendClock(int time) {// seconds
        for (MapleCharacter chr : chars) {
            chr.getClient().announce(Clock.Created(time, 2));
        }
    }
    ...
    public void restartEventTimer(long time) {
        try {
            ...
            for (MapleCharacter chr : getPlayers()) {
                chr.getClient().announce(Clock.Created(timesend, 2));
            }
            timeOut(time, this);
    ...
    public final boolean disposeIfPlayerBelow(final byte size, final int towarp) {
        ...
        if (towarp > 0) {
            if (getMapFactory() == null) {
                System.out.println("mapfactory is null");
            }
            if (em == null) {
                System.out.println("EM is null");
            }
            if (em.getChannel() == null) {
                System.out.println("em channel is null");
            }
    ...
    public void dispose(String pq) {
        ...
        if (pq != null) {
            getEm().setProperty(pq + "Open", "true");
        }
        chars.clear();
        mobs.clear();
        killCount.clear();
        if (mapFactory != null && instanced) {
            mapFactory.clearMaps(instanced);
        }
        mapFactory = null;
        if (expedition != null) {
    ...
    public void openUI(int mapid, int type) {
        this.getMapFactory().getMap(em.getChannel().getId(), mapid).announce(UserLocal.openUI((byte) type));
    }

    public void closeUI(int map) {
        this.getMapFactory().getMap(em.getChannel().getId(), map).announce(UserLocal.SetStandAloneMode(false));
    }
    ...
    public void endCeremony() {
        ...
        mc1.getClient().announce(WvsContext.MarriageResult.Done(mc1, true, mc1.getMarriageID()));
        mc2.getClient().announce(WvsContext.MarriageResult.Done(mc2, true, mc2.getMarriageID()));
        mc1.getMap().announce(Wedding.Progress(true, mc1.getId(), mc2.getId()));
    }

    public void removeCurrentWedding(boolean cathedral) {
        if (cathedral) {
            em.getChannel().setCurrentCathedralMarriageID(-1);
        } else {
            em.getChannel().setCurrentChapelMarriageID(-1);
        }
    }
