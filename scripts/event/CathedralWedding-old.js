...
function playerEntry(eim, player) {
    ...
    //player.getClient().announce(Packages.tools.MaplePacketCreator.getClock(1200));
    //player.getClient().announce(Packages.tools.MaplePacketCreator.getClock(180));
    player.getClient().announce(Packages.tools.packets.CField.Clock.Created(2, (Long.parseLong(eim.getProperty("entryTimestamp")) - System.currentTimeMillis()) / 1000));
}
...
function playerWarpAltar(eim, player) {
    if ((player.getName() != eim.getProperty("husband")) && (player.getName() != eim.getProperty("wife"))){
        ...
        player.getClient().announce(Packages.tools.packets.CField.Clock.Created(2, 300));
    }else{
        player.changeMap(altarMap, altarMap.getPortal(2));
        player.getClient().announce(Packages.tools.packets.CField.Clock.Created(2, 300));
        player.getClient().announce(Packages.tools.packets.WvsContext.BroadcastMsg.encode(6, "Please talk to High Priest John now!"));
    }
}
