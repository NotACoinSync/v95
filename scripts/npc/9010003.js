...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			//Royal Hair Coupon
			if (selection == 100) {
				if (cm.getPlayer().getClient().getRewardPoints() >= 5) {	
			...
			//Transparent Shield	
				...
				if (cm.getPlayer().getClient().getRewardPoints() >= 25) {	
			...
			if (cm.getPlayer().getClient().getRewardPoints() >= 2) {	
			...
			//5 Boxes
				...
				if (cm.getPlayer().getClient().getRewardPoints() >= 10) {	
