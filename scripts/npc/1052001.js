...
function action(mode, type, selection) {
    ...
    if (actionx["1stJob"]){
		...
		if(!cm.isQuestStarted(100009)) cm.startQuest(100009);
            ...
            if (!cm.haveItem(4031012)){
                if (cm.canHold(4031011)){
                    ...
                    cm.sendNextPrev("Collect 30 Dark Marbles. \r\nGive them to the job instructor best of luck!.");
					cm.gainItem(4031011, -1); //Remove on warp?
                } else {
		...
		cm.getPlayer().changeMap(211000001);
        cm.dispose();
    }
}