    ...
    if (mode == -1)
        ...
        if (status == 0) {
            ...
            if (selection == 0) {
                ...
                if (cm.getMeso() >= 35000000 && cm.isQuestCompleted(100201)) {
                    cm.sendOk("That will be 35,000,000 Mesos \r\nUse it with caution, They are hard to get!\r\nTake care now.");
					cm.gainItem(4001017, 5);
					cm.gainMeso(-35000000);	
					cm.dispose();
				}					
                if (cm.getMeso() <= 35000000) {
                    cm.sendOk("Make sure you have enough mesos.");
                    cm.dispose();
                }
		  } else if(selection == 3) {
                if (cm.getMeso() >= 100000000 && cm.canHold(4001017)) {
                    cm.sendOk("Okay well, #eDon't let anyone know about it~#n There!\r\nTake #e#rEye of fire (5)#k#n with you You may need it!");
                ...
                if (cm.getMeso() <= 100000000) {
                    cm.sendOk("Bribery is a sin! \r\bI may not accept that.");
                    cm.dispose();
				}
				if (!cm.isQuestCompleted(100201)) {
				cm.sendOk("You've already Completed all the pre-quests.");
                    cm.dispose();
					}
