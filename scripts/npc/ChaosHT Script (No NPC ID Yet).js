...
var player;
var em;
var exped = MapleExpeditionType.CHAOSHORNTAIL;
var expedName = "ChaosHorntail";
...
function action(mode, type, selection) {
    ...
    em = cm.getEventManager("ChaosHorntailBattle");
    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            if (player.getLevel() < exped.getMinLevel() && player.getLevel() > exped.getMaxLevel()) { //Don't fit requirement
                ...
                cm.dispose();
            } else if (player.getLinkedTotal() <exped.getMinLink() && player.getLinkedTotal() > exped.getMaxLink()) { //Don't fit requirement for Link Level
                cm.sendOk("You do not meet the correct Link Level to battle " + expedBoss + "!");
                cm.dispose();
