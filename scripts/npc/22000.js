...
function action(mode, type, selection) {
    ...
    if (status == 1) {
        ...
        if (cm.haveItem(4031801))
             cm.sendNextPrev("Good job!, your #bspeedrun#k has been saved!");
        else
        if (cm.getLevel() > 6) {
            if (cm.getMeso() < 150) {
        ...
        if (cm.haveItem(4031801)) {
			...
			cm.getPlayer().FinishMapleIsland();
			cm.PlaySong("Coconut/Victory");
			cm.showEffect("event/coconut/victory");
        } else {
        cm.gainMeso(-150);
		cm.getPlayer().DeleteMapleIsland();
		cm.PlaySong("Coconut/Failed");
        }
        cm.warp(104000000, 0);
		cm.gainItem(1002419);
		cm.speedRanks();
