...
var eventManagerName = "HorntailBattle";
var chaosLinkLvlMin = 1500;
...
function action(mode, type, selection) {
    ...
    if (player.getClient().getChannel() == 3)  { // Chaos bosses can only be done on ch3 for now.
        exped = MapleExpeditionType.CHAOS_HORNTAIL;
        expedName = "Chaos Horntail";
        expedBoss = "Chaos Horntail";
        eventManagerName = "ChaosHorntailBattle";
    } else {
        exped = MapleExpeditionType.HORNTAIL;
        expedName = "Horntail";
        expedBoss = "mighty Horntail";
        eventManagerName = "HorntailBattle";
    }
    
    expedition = cm.getExpedition(exped);
    em = cm.getEventManager(eventManagerName);
    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            if (player.getLevel() < exped.getMinLevel() && player.getLevel() > exped.getMaxLevel()) { //Don't fit requirement
                ...
                cm.dispose();
            } else if (player.getClient().getChannel() == 3 && player.getLinkedTotal() < chaosLinkLvlMin) {
                cm.sendOk("You do not meet the minimum link level of 1500 to battle " + expedBoss + "!");
                cm.dispose();
